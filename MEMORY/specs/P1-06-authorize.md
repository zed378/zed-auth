...
- **FR-10** Store codes in Redis with a sub-60-second TTL; redemption must be atomic (`docs/PLAN/04` § What Is Deliberately Not Stored Here).
- **FR-11** Distinguish the silent-SSO path from the login-required path in metrics.

## 4. Non-Functional Requirements

- **NFR-1** The silent-SSO path meets `docs/PLAN/12`: p50 < 50ms, p95 < 150ms, p99 < 300ms. Its budget is one session lookup (a Redis round trip after `P1-11`), one application read, and one Redis write.
...
**None.** Codes and pending requests live in Redis with short TTLs, per `docs/PLAN/04` § What Is Deliberately Not Stored Here: single-use and very short-lived, so durability across a restart is not required, and an automatic expiry is stronger than a cleanup job that can fail silently.
...
The `Store` is Redis-backed. `RedeemCode` is a single `GETDEL`, which is atomic by construction — two concurrent redemptions yield exactly one success, as `docs/PLAN/04` requires. Doing it as `GET` then `DEL` would be the classic replay window.
...
`docs/PLAN/05` says PKCE is "Mandatory for all clients (not just public clients)", which is stricter than OAuth 2.1's baseline. A confidential client omitting `code_challenge` is rejected exactly like a public one.
...
|---|---|---|---|
| A-1 | Open redirect via an unregistered `redirect_uri` | `docs/SECURITY/02` §1 | Phase-1 validation; no redirect before the target is validated |
| A-2 | Open redirect via an error response | Same | The same ordering — this is the subtle half, and the reason the phases exist |
| A-3 | Authorization code interception without the verifier | `docs/PLAN/10` | PKCE mandatory for all client types |
| A-4 | Code replay | `docs/PLAN/10` | Atomic `GETDEL`; one success per code |
...
| A-7 | Missing `state` leaving the client with no CSRF defence | `docs/SECURITY/02` §5 | Required at the endpoint |
...
| A-10 | Codes accumulate and are guessable | `docs/PLAN/04` | 256-bit random, sub-60s TTL, single use |
...
Metrics: `auth_authorize_total{outcome,path}` where path ∈ {`silent`, `login_required`, `error`}, and `auth_authorize_duration_seconds{path}` — separate because `docs/PLAN/12` sets a target for the silent path specifically and an average across both would hide it.
...
The concurrency test matters more than it looks. `docs/PLAN/04` requires atomic redemption, and a `GET`-then-`DEL` implementation passes every sequential test.
