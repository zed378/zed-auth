...
**Fixed**
- **The audit log was not append-only on staging.** `events` and its four live partitions carried `auth_app=arwd` — the service role could rewrite the record of what it had done. Both migrations that revoke those privileges read correctly and a partition created today is fine; the cause is that **golang-migrate never runs an applied version again**, so a database migrated before the `REVOKE` was written keeps what `ALTER DEFAULT PRIVILEGES` granted at `CREATE TABLE`. `20260910000019` repairs the parent and every partition and **refuses to finish** if any relation is still writable. `check.sh` gained a gate beside the row-level-security one, proven to fail by granting `UPDATE` locally. Found by a smoke test, not by the suite. (`P1-20`)
- **"Append-only at the database level" is imprecise** and now says so: it is append-only **for the application role**. The owner retains `UPDATE` and `DELETE`, necessarily — it runs the migrations — so anyone holding the owner DSN can rewrite history, and the deploy path uses it. (`P1-20`)
