...
`PLAN/20` § Recommended Stack lists a marketing static-site generator and a docs framework as separate rows, and `P0-01` step 5 restates that as two things to choose. Read as an instruction, it is two projects.

`UI-UX/20` § Cross-Page Requirements pulls the other way: every page shares the same header and footer, "so moving between Landing → Docs → About never feels like a different product."
...
The deciding argument was not convenience. It was that `PLAN/20` § Versioning Strategy makes docs versioning a requirement and `P0-18`'s Definition of Done asks for it demonstrated — and the obvious marketing-side alternative, Astro with Starlight, has no native versioning. Choosing a stack that needs a third-party plugin for a stated requirement is the wrong trade.

---

## What Was Built

The full route structure from `PLAN/20`: `/`, `/about`, `/contact`, `/docs` with quickstart, concepts, guides, console and a generated API reference, and `/changelog`. Thirty-four pages.

`/pricing` does not exist — `PLAN/20` says omit entirely unless a commercial tier exists. `/security` does not exist either, because `PLAN/20` § Deployment & Roadmap Placement puts the trust page alongside Phase 5; `/contact` carries the responsible-disclosure path in the meantime, which `PLAN/20` requires to exist. A researcher with nowhere to report goes public instead.
...
**Search** is local and indexed at build time — 123 documents. `UI-UX/20` requires fuzzy matching because "a developer often doesn't know the exact terminology this project uses yet".

---

## The Content Rule, Made Structural

`UI-UX/21` § Content Governance and `CLAUDE.md` both forbid present-tense copy for an unshipped capability. `UI-UX/21`'s own landing-page blueprint has four capability cards — SSO, REST API, RBAC, policies — written in the present tense, and none of those four exists. The project is in Phase 0: the service runs and serves two operational probes.

So the landing page describes them as the design, each labelled with the roadmap phase that delivers it, under a heading that says so. The hero keeps `UI-UX/21`'s headline verbatim, because it describes what the product is *for* rather than what you can do with it today.

Two smaller judgements followed from the same rule:

- **No social-proof section.** `UI-UX/20` is explicit — include it "only once genuinely available", because "an empty or fabricated social-proof section is worse than omitting it entirely". There is nobody to quote.
- **The hero CTA points at the docs, not a quickstart.** `UI-UX/21`'s blueprint says "Get Started". A "Get Started" button leading to a page that says "not available yet" costs more trust than it wins.
...
`PLAN/20` § Why a Separate Surface says these two surfaces share the visual language and nothing else. That is two rules, and both erode quietly rather than by decision.
...
Measured against the dark surface, that palette gives: accent **2.4:1**, danger **2.7:1**, warning **3.3:1**, success **2.8:1** — every one below the 4.5:1 `UI-UX/13` requires — and the border at **1.8:1**, below the 3:1 WCAG 1.4.11 wants of a component boundary.
...
| `/pricing`, `/security` | absent, per `PLAN/20` |
...
- **The Lighthouse bar in `P0-18`'s Definition of Done does not exist.** It points at `UI-UX/20` § Cross-Page Requirements, which sets an accessibility bar and no numeric performance target. Nothing has been measured against a number, because there is no number. Worth settling in `P0-19` or deleting from the DoD.
- **Analytics is not wired.** `PLAN/20` asks for privacy-respecting page analytics; every option is a third party or a service to run, which is a decision rather than a default. Deferred to `P0-19` with the content it would measure.
