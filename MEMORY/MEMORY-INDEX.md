...
| 2026-09-08 | P0-18 | [The public site](./records/2026-09-08-P0-18-public-site-skeleton.md) | Docs versioning, local search, and an API reference generated from the same spec the backend and console are generated from. `docs/PLAN/20` and `docs/UI-UX/20` wanted different things; ADR-014 records which won. A dark mode that would have shipped unreadable, caught by computing contrast rather than looking |
...
| 2026-09-10 | P1-23, P1-24 | [Users and audit log](./records/2026-09-10-P1-23-P1-24-console-users-audit.md) | Flow 1 keeps step 2 and gates it behind an explicit "no access yet" checkbox, because dropping the step would remove the moment where access is considered at all. Unavailable is not empty. The API client captured `fetch` at module load, which is a production bug as well as a test one — and the harness I wrote to catch it answered every call with the success body, including the POST whose failure the test existed to check |
| 2026-09-10 | P1-22 | [Console screens](./records/2026-09-10-P1-22-console-screens.md) | The components were the larger half, which is the shared-table rule paying immediately. The four states live in one file because they are a set, and both distinctions — filtered vs genuinely empty, retryable vs refused — are enforced by the types. The client secret dialog resists Escape and gates Done behind an acknowledgement, because the service genuinely cannot show it again. `color-danger` was tempting in two places that are not destructive |
| 2026-09-10 | P1-21 | [Console login](./records/2026-09-10-P1-21-console-login.md) | The measure of success is that it changed no backend code — the dogfooding constraint fails quietly, as one endpoint only the console calls. ADR-019 keeps the token in memory and recovers it with prompt=none; the claim is stated precisely rather than oversold, because in-memory is not immune to XSS, it removes persistence. The route guard says three times that it is not a control. A test hook nearly went into the one file that decides where authorization codes are sent |
| 2026-09-10 | P1-20 | [Audit log read](./records/2026-09-10-P1-20-audit-log-read.md) | One operation, asserted against what the ROUTER registers rather than what the package implements, and backed by the application role being unable to UPDATE or DELETE the table at all. Newest first, alone in this API, with the id in the cursor because events is partitioned and its key is the pair. Eleven filter combinations run against a neighbour seeded with the same type, actor and instant |
| 2026-09-10 | P1-19 | [Users](./records/2026-09-10-P1-19-users.md) | No password crosses this API in either direction, and the reset link is not in the response to the administrator who triggered it. The forgot endpoint's two answers are compared on BYTES, with a control proving one message was actually sent. Deactivation is three revocations plus a cache tombstone, tested through a real session and a real refresh token. Single use is one conditional UPDATE, raced by sixteen goroutines. ADR-018 closes OQ-04; PG-18 closes narrowly — an invitation verifies the address, a reset does not |
| 2026-09-10 | P1-18 | [Applications](./records/2026-09-10-P1-18-applications.md) | The secret leaves the process exactly twice, and the test asserts its characters appear in no other body rather than that a field is absent. Two boundaries the store could not know about: the project, which RLS cannot hold because `project_id` is not a tenant column, and P1-15's audit guard, which the store bypassed — a false alarm on every normal request is worse than a missing one. A caller's typo was a 500 because validation errors were told apart by matching on text |
| 2026-09-10 | P1-17 | [Projects](./records/2026-09-10-P1-17-projects.md) | No migration and no new control — the API in front of a table `P0-07` already got right. The tenant is enforced by shape rather than by check: `store.go` has no `org_id` parameter anywhere, which is also what makes the RLS test honest, since a predicate would make it pass whether or not the policy existed. Deletion refuses with the counts of what blocks it rather than with a foreign key's error message |
| 2026-09-10 | P1-16 | [Organizations](./records/2026-09-10-P1-16-organizations.md) | The first real `/v1` surface, and the endpoint that found `P1-15`'s cross-tenant reads returning an empty result rather than a refusal. `organizations` is the one table whose RLS is keyed on `id`, so listing and creating need SECURITY DEFINER functions that take no organization id. Three contract gaps made visible rather than quiet: prefixed ids versus uuids, a self-contradicting `Forbidden` description, and `additionalProperties: false` being documentation |
| 2026-09-10 | P1-15 | [Management API foundation](./records/2026-09-10-P1-15-management-api-foundation.md) | Every default is a refusal: `Requirement`'s zero value is unsatisfiable, so an unannotated route is unreachable rather than open. Roles are read from the database on every request, never from the token. Found a `404`-not-`403` control that existed only as a comment, a sweep that deleted nothing under RLS and reported success, and two bugs in the verification tooling that were making checks pass for no reason |
| 2026-09-10 | P1-14 | [Authentication audit events](./records/2026-09-10-P1-14-authentication-audit.md) | Mostly verification, and it found that `P1-13`'s lockout event was never written: `WithTenant("")` returns an error and the error was discarded. The DoD item had been ticked on a fake auditor - a check at the wrong layer for the claim it was supporting |
| 2026-09-10 | P1-13 | [Login rate limiting](./records/2026-09-10-P1-13-rate-limiting.md) | Counters keyed on the SUBMITTED ADDRESS rather than a resolved user, which is what lets the refusal say something true without confirming an account exists. A mutation found dead code in `Evaluate` that read like the allowance check and decided nothing. ADR-017: fail open, loudly |
| 2026-09-10 | P1-10 | [RP-initiated logout](./records/2026-09-10-P1-10-logout.md) | A GET logs somebody out only when the request proves it came from a party holding a token for THAT session; everything else is a confirmation click. Clearing the cookie is done as well as revoking the row, never instead — and the test asserts the consequence at /oauth/authorize rather than a revoked_at column |
| 2026-09-10 | P1-09 | [Token introspection and revocation](./records/2026-09-10-P1-09-introspect-revoke.md) | A client may only see or destroy its own tokens, and another client's is answered as unknown rather than forbidden. Two mutations were my own mistakes and both taught something: one substitute was not a weakening, and the other found that audit redaction matches key NAMES, so a leak under a novel key sails through |
| 2026-09-10 | P1-08 | [The userinfo endpoint](./records/2026-09-10-P1-08-userinfo.md) | The scope mapping is tested for ABSENCE with an exact key set — a handler returning every claim regardless of scope passes any presence test. Session liveness is checked because it rides free in the user read. A mutation showed the end-to-end id_token test was passing for the audience reason, not the typ reason; the fix was to correct the claim, not weaken the design |
| 2026-09-10 | — | [Reference documentation moved under `docs/`](./records/2026-09-10-reference-docs-under-docs.md) | The AI-execution structure from `wedding-saas`. The three doc folders turned out to be self-contained — 44 links, every one a sibling — so moving them together needed zero edits inside them. `go vet ./...` does not compile build-tagged files, so it passed while the integration build did not |
| 2026-09-09 | P1-12 | [The hosted login page](./records/2026-09-09-P1-12-login-page.md) | The last piece before a human can log in. The uniform-answer requirement is tested by submitting the SAME address twice with the account deleted in between — comparing two different addresses would have forced the assertion to be loosened until it tested nothing. Consuming the pending request on every submission would have given every account exactly one attempt at its password |
| 2026-09-09 | P1-07 | [The token endpoint](./records/2026-09-09-P1-07-token.md) | The flow closes: a consumer can complete a login, and `P1-04`'s deferred DoD items become true. The refresh token is opaque rather than a JWT partly because `P1-03`'s malleability finding would undermine `P3-06`'s reuse detection. Found a client authenticator that checked a secret was present and never checked it |
...
| 2026-09-08 | — | [Phase F frontend track added](./records/2026-09-08-phase-f-frontend-track.md) | 53 frontend tasks; resolves the tension with docs/PLAN/16's lockstep rule by gating each page on its backend task; found a 4-screen omission in docs/UI-UX/08 |
| 2026-09-08 | — | [Plan gap remediation](./records/2026-09-08-plan-gap-remediation.md) | All 11 gaps and both contradictions closed by amending the plan; docs/PLAN/04 grew 158 → 295 lines; 2 further gaps found while fixing them |
...
- `P1-07` — [The token endpoint](./records/2026-09-09-P1-07-token.md) — consume the single-use credential before checking anything else
