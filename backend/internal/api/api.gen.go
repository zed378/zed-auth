...
// Defines values for ProjectGrantStatus.
const (
	ProjectGrantStatusActive  ProjectGrantStatus = "active"
	ProjectGrantStatusRevoked ProjectGrantStatus = "revoked"
)
...
type Application struct {
	...
	// Saml The SAML service provider an application of type `saml` represents.
	//
	...
	Saml *SamlRegistration `json:"saml,omitempty"`
...
type ApplicationCreate struct {
	...
	// Saml Required when `type` is `saml`, and refused otherwise.
	//
	...
	// matched to it.
	Saml *SamlRegistrationInput `json:"saml,omitempty"`
...
type ApplicationUpdate struct {
	...
	// Saml Replaces the whole registration, on an application of type `saml`
	// and no other.
	//
...
type ProjectGrant struct {
	...
	// GrantedOrgId A resource's stable identifier. Not sequential and not guessable.
	//
	// **This was specified as a prefixed, sortable identifier** — `usr_`,
	// `org_`, `prj_` — and is a UUID instead. The change is deliberate and is
	// recorded as `PG-23`.
	...
	// So: UUIDs everywhere, and if prefixed identifiers are wanted later they
	// arrive everywhere at once or not at all.
	GrantedOrgId ResourceId `json:"granted_org_id"`

	// GrantedOrgName The receiving organization's name, so a person revoking can check who
	// the grant is to. Resolved only for organizations holding a grant from
	// this one.
	GrantedOrgName  string    `json:"granted_org_name"`
	...
	Status    ProjectGrantStatus           `json:"status"`
}

// ProjectGrantStatus defines model for ProjectGrant.Status.
type ProjectGrantStatus string

// ProjectGrantCreate defines model for ProjectGrantCreate.
type ProjectGrantCreate struct {
	...
	// RoleKeys Roles of this project the receiving organization may assign. Every one must exist in this project.
	RoleKeys []RoleKey `json:"role_keys"`
}

// ProjectGrantList defines model for ProjectGrantList.
...
// ProjectGrantOwner defines model for ProjectGrantOwner.
type ProjectGrantOwner struct {
	CreatedAt time.Time `json:"created_at"`

	// GrantId A resource's stable identifier. Not sequential and not guessable.
	//
	// **This was specified as a prefixed, sortable identifier** — `usr_`,
	// `org_`, `prj_` — and is a UUID instead. The change is deliberate and is
	// recorded as `PG-23`.
	...
	// So: UUIDs everywhere, and if prefixed identifiers are wanted later they
	// arrive everywhere at once or not at all.
	GrantId ResourceId `json:"grant_id"`
...
// ProjectGrantOwnerCreate defines model for ProjectGrantOwnerCreate.
type ProjectGrantOwnerCreate struct {
	// UserId A resource's stable identifier. Not sequential and not guessable.
	//
	// **This was specified as a prefixed, sortable identifier** — `usr_`,
	// `org_`, `prj_` — and is a UUID instead. The change is deliberate and is
...
// ProjectGrantOwnerList defines model for ProjectGrantOwnerList.
type ProjectGrantOwnerList struct {
	Owners []ProjectGrantOwner `json:"owners"`
}
...
// Role A named bundle of permission keys, belonging to exactly one project.
//
...
type Role struct {
	...
	// GrantCount How many user grants reference this role. It is what a delete would
	...
	// IsBuiltin A role the service owns. It cannot be deleted or re-keyed, by any
	// caller. No built-in roles are seeded today — see `PG-30`.
	IsBuiltin bool `json:"is_builtin"`

	// Key A role's stable identifier, unique within its project. `admin` in one
	...
	// PermissionKeys What the role carries. May be empty: a role with no permissions is
...
type RoleCreate struct {
	...
	// PermissionKeys Optional. Duplicates are **refused**, not folded: silently changing
...
// RoleUpdate `key` is absent deliberately and is not editable. See the endpoint.
type RoleUpdate struct {
	DisplayName string `json:"display_name"`

	// PermissionKeys Replaces the set entirely. A partial update of an array is
...
type SamlRegistration struct {
	// AcsUrl Where an assertion is POSTed. `https` only — an assertion is a
	...
	// AllowIdpInitiated Whether this service provider accepts a login it did not ask for.
	//
	...
	// AttributeRelease The attributes this service provider receives in an assertion.
	//
	...
	// CertificateExpiresAt When the service provider's certificate expires. Absent when there
	...
	// CertificateExpiresSoon True within 30 days of expiry. The window is that long because the
	...
	CertificateExpiresSoon *bool `json:"certificate_expires_soon,omitempty"`

	// EntityId The service provider's own name, compared by **exact string
	...
	// WantSignedRequests Whether this service provider's own `AuthnRequest`s must carry a
	// valid signature.
	//
...
// SamlRegistrationInput A SAML registration as supplied by a caller.
//
...
type SamlRegistrationInput struct {
	...
	WantSignedRequests *bool   `json:"want_signed_requests,omitempty"`
}
...
// ProjectGrantId A resource's stable identifier. Not sequential and not guessable.
//
// **This was specified as a prefixed, sortable identifier** — `usr_`,
// `org_`, `prj_` — and is a UUID instead. The change is deliberate and is
// recorded as `PG-23`.
...
// So: UUIDs everywhere, and if prefixed identifiers are wanted later they
// arrive everywhere at once or not at all.
type ProjectGrantId = ResourceId
...
// RoleId A resource's stable identifier. Not sequential and not guessable.
//
// **This was specified as a prefixed, sortable identifier** — `usr_`,
// `org_`, `prj_` — and is a UUID instead. The change is deliberate and is
// recorded as `PG-23`.
...
// RemoveProjectGrantOwnerParams defines parameters for RemoveProjectGrantOwner.
type RemoveProjectGrantOwnerParams struct {
	// IdempotencyKey A client-generated key making a retried `POST` safe. Replaying a
	// request with the same key returns the original result rather than
	// creating a second resource — which matters most for automated
	// provisioning, where a network timeout is indistinguishable from a
...
// ListProjectGrantsParams defines parameters for ListProjectGrants.
type ListProjectGrantsParams struct {
	// PageSize Maximum items to return. The server may return fewer, and returning
	// fewer never means the collection is exhausted — only an absent
	// `next_page_token` means that.
	PageSize *PageSize `form:"page_size,omitempty" json:"page_size,omitempty"`
...
// CreateProjectGrantParams defines parameters for CreateProjectGrant.
...
// AssignProjectGrantOwnerJSONRequestBody defines body for AssignProjectGrantOwner for application/json ContentType.
type AssignProjectGrantOwnerJSONRequestBody = ProjectGrantOwnerCreate
...
type CreateProjectGrantJSONRequestBody = ProjectGrantCreate
...
type ServerInterface interface {
	...
	// (POST /v1/organizations/{org_id}/project-grants/{grant_id}/owners)
	AssignProjectGrantOwner(w http.ResponseWriter, r *http.Request, orgId OrganizationId, grantId ProjectGrantId, params AssignProjectGrantOwnerParams)
	// Remove an owner of a received grant
	// (DELETE /v1/organizations/{org_id}/project-grants/{grant_id}/owners/{user_id})
	RemoveProjectGrantOwner(w http.ResponseWriter, r *http.Request, orgId OrganizationId, grantId ProjectGrantId, userId UserId, params RemoveProjectGrantOwnerParams)
	...
	// (GET /v1/organizations/{org_id}/projects/{project_id}/grants)
	ListProjectGrants(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, params ListProjectGrantsParams)
	// Delegate this project to another organization
	// (POST /v1/organizations/{org_id}/projects/{project_id}/grants)
	CreateProjectGrant(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, params CreateProjectGrantParams)
	// Revoke a grant
	// (DELETE /v1/organizations/{org_id}/projects/{project_id}/grants/{grant_id})
	RevokeProjectGrant(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, grantId ProjectGrantId, params RevokeProjectGrantParams)
	// Read a grant
	// (GET /v1/organizations/{org_id}/projects/{project_id}/grants/{grant_id})
	GetProjectGrant(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, grantId ProjectGrantId)
	// List a project's roles
	// (GET /v1/organizations/{org_id}/projects/{project_id}/roles)
	ListRoles(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, params ListRolesParams)
	// Define a role
	// (POST /v1/organizations/{org_id}/projects/{project_id}/roles)
	CreateRole(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, params CreateRoleParams)
	// Delete a role
	// (DELETE /v1/organizations/{org_id}/projects/{project_id}/roles/{role_id})
	DeleteRole(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, roleId RoleId)
	// Read a role
	// (GET /v1/organizations/{org_id}/projects/{project_id}/roles/{role_id})
	GetRole(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, roleId RoleId)
	// Change a role's label or permissions
	// (PATCH /v1/organizations/{org_id}/projects/{project_id}/roles/{role_id})
	UpdateRole(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, roleId RoleId)
...
// List who may assign a received grant's roles
// (GET /v1/organizations/{org_id}/project-grants/{grant_id}/owners)
func (_ Unimplemented) ListProjectGrantOwners(w http.ResponseWriter, r *http.Request, orgId OrganizationId, grantId ProjectGrantId) {
	w.WriteHeader(http.StatusNotImplemented)
}

// Make one of this organization's users an owner of a received grant
// (POST /v1/organizations/{org_id}/project-grants/{grant_id}/owners)
func (_ Unimplemented) AssignProjectGrantOwner(w http.ResponseWriter, r *http.Request, orgId OrganizationId, grantId ProjectGrantId, params AssignProjectGrantOwnerParams) {
	w.WriteHeader(http.StatusNotImplemented)
}

// Remove an owner of a received grant
// (DELETE /v1/organizations/{org_id}/project-grants/{grant_id}/owners/{user_id})
func (_ Unimplemented) RemoveProjectGrantOwner(w http.ResponseWriter, r *http.Request, orgId OrganizationId, grantId ProjectGrantId, userId UserId, params RemoveProjectGrantOwnerParams) {
	w.WriteHeader(http.StatusNotImplemented)
}
...
// List the grants that delegate this project
// (GET /v1/organizations/{org_id}/projects/{project_id}/grants)
func (_ Unimplemented) ListProjectGrants(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, params ListProjectGrantsParams) {
	w.WriteHeader(http.StatusNotImplemented)
}

// Delegate this project to another organization
// (POST /v1/organizations/{org_id}/projects/{project_id}/grants)
func (_ Unimplemented) CreateProjectGrant(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, params CreateProjectGrantParams) {
	w.WriteHeader(http.StatusNotImplemented)
}

// Revoke a grant
// (DELETE /v1/organizations/{org_id}/projects/{project_id}/grants/{grant_id})
func (_ Unimplemented) RevokeProjectGrant(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, grantId ProjectGrantId, params RevokeProjectGrantParams) {
	w.WriteHeader(http.StatusNotImplemented)
}

// Read a grant
// (GET /v1/organizations/{org_id}/projects/{project_id}/grants/{grant_id})
func (_ Unimplemented) GetProjectGrant(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, grantId ProjectGrantId) {
	w.WriteHeader(http.StatusNotImplemented)
}

// List a project's roles
// (GET /v1/organizations/{org_id}/projects/{project_id}/roles)
func (_ Unimplemented) ListRoles(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, params ListRolesParams) {
	w.WriteHeader(http.StatusNotImplemented)
}

// Define a role
// (POST /v1/organizations/{org_id}/projects/{project_id}/roles)
func (_ Unimplemented) CreateRole(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, params CreateRoleParams) {
	w.WriteHeader(http.StatusNotImplemented)
}

// Delete a role
// (DELETE /v1/organizations/{org_id}/projects/{project_id}/roles/{role_id})
func (_ Unimplemented) DeleteRole(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, roleId RoleId) {
	w.WriteHeader(http.StatusNotImplemented)
}

// Read a role
// (GET /v1/organizations/{org_id}/projects/{project_id}/roles/{role_id})
func (_ Unimplemented) GetRole(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, roleId RoleId) {
	w.WriteHeader(http.StatusNotImplemented)
}

// Change a role's label or permissions
// (PATCH /v1/organizations/{org_id}/projects/{project_id}/roles/{role_id})
func (_ Unimplemented) UpdateRole(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, roleId RoleId) {
	w.WriteHeader(http.StatusNotImplemented)
}
...
// ListProjectGrantOwners operation middleware
func (siw *ServerInterfaceWrapper) ListProjectGrantOwners(w http.ResponseWriter, r *http.Request) {
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.ListProjectGrantOwners(w, r, orgId, grantId)
	}))
...
// AssignProjectGrantOwner operation middleware
func (siw *ServerInterfaceWrapper) AssignProjectGrantOwner(w http.ResponseWriter, r *http.Request) {
	...
	// Parameter object where we will unmarshal all parameters from the context
	var params AssignProjectGrantOwnerParams
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.AssignProjectGrantOwner(w, r, orgId, grantId, params)
	}))
...
// RemoveProjectGrantOwner operation middleware
func (siw *ServerInterfaceWrapper) RemoveProjectGrantOwner(w http.ResponseWriter, r *http.Request) {
	...
	// Parameter object where we will unmarshal all parameters from the context
	var params RemoveProjectGrantOwnerParams
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.RemoveProjectGrantOwner(w, r, orgId, grantId, userId, params)
	}))
...
func (siw *ServerInterfaceWrapper) ListProjectGrants(w http.ResponseWriter, r *http.Request) {
	...
	// Parameter object where we will unmarshal all parameters from the context
	var params ListProjectGrantsParams
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.ListProjectGrants(w, r, orgId, projectId, params)
	}))
...
func (siw *ServerInterfaceWrapper) CreateProjectGrant(w http.ResponseWriter, r *http.Request) {
	...
	// Parameter object where we will unmarshal all parameters from the context
	var params CreateProjectGrantParams
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.CreateProjectGrant(w, r, orgId, projectId, params)
	}))
...
func (siw *ServerInterfaceWrapper) RevokeProjectGrant(w http.ResponseWriter, r *http.Request) {
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.RevokeProjectGrant(w, r, orgId, projectId, grantId, params)
	}))
...
// GetProjectGrant operation middleware
func (siw *ServerInterfaceWrapper) GetProjectGrant(w http.ResponseWriter, r *http.Request) {
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.GetProjectGrant(w, r, orgId, projectId, grantId)
	}))
...
func (siw *ServerInterfaceWrapper) ListRoles(w http.ResponseWriter, r *http.Request) {
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.ListRoles(w, r, orgId, projectId, params)
	}))
...
func (siw *ServerInterfaceWrapper) CreateRole(w http.ResponseWriter, r *http.Request) {
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.CreateRole(w, r, orgId, projectId, params)
	}))
...
func (siw *ServerInterfaceWrapper) DeleteRole(w http.ResponseWriter, r *http.Request) {
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.DeleteRole(w, r, orgId, projectId, roleId)
	}))
...
func (siw *ServerInterfaceWrapper) GetRole(w http.ResponseWriter, r *http.Request) {
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.GetRole(w, r, orgId, projectId, roleId)
	}))
...
func (siw *ServerInterfaceWrapper) UpdateRole(w http.ResponseWriter, r *http.Request) {
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.UpdateRole(w, r, orgId, projectId, roleId)
	}))
...
func HandlerWithOptions(si ServerInterface, options ChiServerOptions) http.Handler {
	...
	r.Group(func(r chi.Router) {
		r.Post(options.BaseURL+"/v1/organizations/{org_id}/project-grants/{grant_id}/owners", wrapper.AssignProjectGrantOwner)
	})
	r.Group(func(r chi.Router) {
		r.Delete(options.BaseURL+"/v1/organizations/{org_id}/project-grants/{grant_id}/owners/{user_id}", wrapper.RemoveProjectGrantOwner)
	})
	r.Group(func(r chi.Router) {
	...
	r.Group(func(r chi.Router) {
		r.Get(options.BaseURL+"/v1/organizations/{org_id}/projects/{project_id}/grants", wrapper.ListProjectGrants)
	})
	r.Group(func(r chi.Router) {
		r.Post(options.BaseURL+"/v1/organizations/{org_id}/projects/{project_id}/grants", wrapper.CreateProjectGrant)
	})
	r.Group(func(r chi.Router) {
		r.Delete(options.BaseURL+"/v1/organizations/{org_id}/projects/{project_id}/grants/{grant_id}", wrapper.RevokeProjectGrant)
	})
	r.Group(func(r chi.Router) {
		r.Get(options.BaseURL+"/v1/organizations/{org_id}/projects/{project_id}/grants/{grant_id}", wrapper.GetProjectGrant)
	})
	r.Group(func(r chi.Router) {
		r.Get(options.BaseURL+"/v1/organizations/{org_id}/projects/{project_id}/roles", wrapper.ListRoles)
	})
	r.Group(func(r chi.Router) {
		r.Post(options.BaseURL+"/v1/organizations/{org_id}/projects/{project_id}/roles", wrapper.CreateRole)
	})
	r.Group(func(r chi.Router) {
		r.Delete(options.BaseURL+"/v1/organizations/{org_id}/projects/{project_id}/roles/{role_id}", wrapper.DeleteRole)
	})
	r.Group(func(r chi.Router) {
		r.Get(options.BaseURL+"/v1/organizations/{org_id}/projects/{project_id}/roles/{role_id}", wrapper.GetRole)
	})
	r.Group(func(r chi.Router) {
		r.Patch(options.BaseURL+"/v1/organizations/{org_id}/projects/{project_id}/roles/{role_id}", wrapper.UpdateRole)
	})
	r.Group(func(r chi.Router) {
...
type ListProjectGrantOwnersRequestObject struct {
	OrgId   OrganizationId `json:"org_id"`
	GrantId ProjectGrantId `json:"grant_id"`
}

type ListProjectGrantOwnersResponseObject interface {
	VisitListProjectGrantOwnersResponse(w http.ResponseWriter) error
}

type ListProjectGrantOwners200JSONResponse ProjectGrantOwnerList

func (response ListProjectGrantOwners200JSONResponse) VisitListProjectGrantOwnersResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
...
type ListProjectGrantOwners401JSONResponse struct{ UnauthorizedJSONResponse }

func (response ListProjectGrantOwners401JSONResponse) VisitListProjectGrantOwnersResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(401)
...
type ListProjectGrantOwners403JSONResponse struct{ ForbiddenJSONResponse }

func (response ListProjectGrantOwners403JSONResponse) VisitListProjectGrantOwnersResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(403)
...
type ListProjectGrantOwners404JSONResponse struct{ NotFoundJSONResponse }

func (response ListProjectGrantOwners404JSONResponse) VisitListProjectGrantOwnersResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(404)
...
type ListProjectGrantOwners429JSONResponse struct{ RateLimitedJSONResponse }

func (response ListProjectGrantOwners429JSONResponse) VisitListProjectGrantOwnersResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Retry-After", fmt.Sprint(response.Headers.RetryAfter))
	w.Header().Set("X-RateLimit-Limit", fmt.Sprint(response.Headers.XRateLimitLimit))
	w.Header().Set("X-RateLimit-Remaining", fmt.Sprint(response.Headers.XRateLimitRemaining))
...
type ListProjectGrantOwners500JSONResponse struct{ InternalErrorJSONResponse }

func (response ListProjectGrantOwners500JSONResponse) VisitListProjectGrantOwnersResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(500)
...
type AssignProjectGrantOwnerRequestObject struct {
	OrgId   OrganizationId `json:"org_id"`
	GrantId ProjectGrantId `json:"grant_id"`
	Params  AssignProjectGrantOwnerParams
	Body    *AssignProjectGrantOwnerJSONRequestBody
}

type AssignProjectGrantOwnerResponseObject interface {
	VisitAssignProjectGrantOwnerResponse(w http.ResponseWriter) error
}

type AssignProjectGrantOwner201JSONResponse ProjectGrantOwner

func (response AssignProjectGrantOwner201JSONResponse) VisitAssignProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(201)
...
type AssignProjectGrantOwner400JSONResponse struct{ BadRequestJSONResponse }

func (response AssignProjectGrantOwner400JSONResponse) VisitAssignProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(400)
...
type AssignProjectGrantOwner401JSONResponse struct{ UnauthorizedJSONResponse }

func (response AssignProjectGrantOwner401JSONResponse) VisitAssignProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(401)
...
type AssignProjectGrantOwner403JSONResponse struct{ ForbiddenJSONResponse }

func (response AssignProjectGrantOwner403JSONResponse) VisitAssignProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(403)
...
type AssignProjectGrantOwner404JSONResponse struct{ NotFoundJSONResponse }

func (response AssignProjectGrantOwner404JSONResponse) VisitAssignProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(404)
...
type AssignProjectGrantOwner409JSONResponse struct{ ConflictJSONResponse }

func (response AssignProjectGrantOwner409JSONResponse) VisitAssignProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(409)
...
type AssignProjectGrantOwner429JSONResponse struct{ RateLimitedJSONResponse }

func (response AssignProjectGrantOwner429JSONResponse) VisitAssignProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Retry-After", fmt.Sprint(response.Headers.RetryAfter))
	w.Header().Set("X-RateLimit-Limit", fmt.Sprint(response.Headers.XRateLimitLimit))
	w.Header().Set("X-RateLimit-Remaining", fmt.Sprint(response.Headers.XRateLimitRemaining))
...
type AssignProjectGrantOwner500JSONResponse struct{ InternalErrorJSONResponse }

func (response AssignProjectGrantOwner500JSONResponse) VisitAssignProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(500)
...
type RemoveProjectGrantOwnerRequestObject struct {
	OrgId   OrganizationId `json:"org_id"`
	GrantId ProjectGrantId `json:"grant_id"`
	UserId  UserId         `json:"user_id"`
	Params  RemoveProjectGrantOwnerParams
}

type RemoveProjectGrantOwnerResponseObject interface {
	VisitRemoveProjectGrantOwnerResponse(w http.ResponseWriter) error
}

type RemoveProjectGrantOwner204Response struct {
}

func (response RemoveProjectGrantOwner204Response) VisitRemoveProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.WriteHeader(204)
	return nil
}

type RemoveProjectGrantOwner401JSONResponse struct{ UnauthorizedJSONResponse }

func (response RemoveProjectGrantOwner401JSONResponse) VisitRemoveProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(401)
...
type RemoveProjectGrantOwner403JSONResponse struct{ ForbiddenJSONResponse }

func (response RemoveProjectGrantOwner403JSONResponse) VisitRemoveProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(403)
...
type RemoveProjectGrantOwner404JSONResponse struct{ NotFoundJSONResponse }

func (response RemoveProjectGrantOwner404JSONResponse) VisitRemoveProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(404)
...
type RemoveProjectGrantOwner429JSONResponse struct{ RateLimitedJSONResponse }

func (response RemoveProjectGrantOwner429JSONResponse) VisitRemoveProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Retry-After", fmt.Sprint(response.Headers.RetryAfter))
	w.Header().Set("X-RateLimit-Limit", fmt.Sprint(response.Headers.XRateLimitLimit))
	w.Header().Set("X-RateLimit-Remaining", fmt.Sprint(response.Headers.XRateLimitRemaining))
...
type RemoveProjectGrantOwner500JSONResponse struct{ InternalErrorJSONResponse }

func (response RemoveProjectGrantOwner500JSONResponse) VisitRemoveProjectGrantOwnerResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(500)
...
type ListProjectGrantsRequestObject struct {
	OrgId     OrganizationId `json:"org_id"`
	ProjectId ProjectId      `json:"project_id"`
	Params    ListProjectGrantsParams
}
...
type ListProjectGrants200JSONResponse ProjectGrantList
...
type ListProjectGrants400JSONResponse struct{ BadRequestJSONResponse }

func (response ListProjectGrants400JSONResponse) VisitListProjectGrantsResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(400)
...
type ListProjectGrants401JSONResponse struct{ UnauthorizedJSONResponse }
...
type ListProjectGrants429JSONResponse struct{ RateLimitedJSONResponse }
...
type ListProjectGrants500JSONResponse struct{ InternalErrorJSONResponse }
...
type CreateProjectGrantRequestObject struct {
	OrgId     OrganizationId `json:"org_id"`
	ProjectId ProjectId      `json:"project_id"`
	Params    CreateProjectGrantParams
...
type CreateProjectGrant201JSONResponse ProjectGrant

func (response CreateProjectGrant201JSONResponse) VisitCreateProjectGrantResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(201)
...
type CreateProjectGrant401JSONResponse struct{ UnauthorizedJSONResponse }
...
type CreateProjectGrant409JSONResponse struct{ ConflictJSONResponse }

func (response CreateProjectGrant409JSONResponse) VisitCreateProjectGrantResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(409)
...
type CreateProjectGrant429JSONResponse struct{ RateLimitedJSONResponse }
...
type CreateProjectGrant500JSONResponse struct{ InternalErrorJSONResponse }
...
type RevokeProjectGrant401JSONResponse struct{ UnauthorizedJSONResponse }
...
type RevokeProjectGrant429JSONResponse struct{ RateLimitedJSONResponse }
...
type RevokeProjectGrant500JSONResponse struct{ InternalErrorJSONResponse }
...
type GetProjectGrantRequestObject struct {
	OrgId     OrganizationId `json:"org_id"`
	ProjectId ProjectId      `json:"project_id"`
	GrantId   ProjectGrantId `json:"grant_id"`
}

type GetProjectGrantResponseObject interface {
	VisitGetProjectGrantResponse(w http.ResponseWriter) error
}

type GetProjectGrant200JSONResponse ProjectGrant

func (response GetProjectGrant200JSONResponse) VisitGetProjectGrantResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
...
type GetProjectGrant401JSONResponse struct{ UnauthorizedJSONResponse }

func (response GetProjectGrant401JSONResponse) VisitGetProjectGrantResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(401)
...
type GetProjectGrant403JSONResponse struct{ ForbiddenJSONResponse }

func (response GetProjectGrant403JSONResponse) VisitGetProjectGrantResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(403)
...
type GetProjectGrant404JSONResponse struct{ NotFoundJSONResponse }

func (response GetProjectGrant404JSONResponse) VisitGetProjectGrantResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(404)
...
type GetProjectGrant429JSONResponse struct{ RateLimitedJSONResponse }

func (response GetProjectGrant429JSONResponse) VisitGetProjectGrantResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Retry-After", fmt.Sprint(response.Headers.RetryAfter))
	w.Header().Set("X-RateLimit-Limit", fmt.Sprint(response.Headers.XRateLimitLimit))
	w.Header().Set("X-RateLimit-Remaining", fmt.Sprint(response.Headers.XRateLimitRemaining))
...
type GetProjectGrant500JSONResponse struct{ InternalErrorJSONResponse }

func (response GetProjectGrant500JSONResponse) VisitGetProjectGrantResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(500)
...
type ListRoles404JSONResponse struct{ NotFoundJSONResponse }

func (response ListRoles404JSONResponse) VisitListRolesResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(404)
...
type ListRoles429JSONResponse struct{ RateLimitedJSONResponse }

func (response ListRoles429JSONResponse) VisitListRolesResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Retry-After", fmt.Sprint(response.Headers.RetryAfter))
	w.Header().Set("X-RateLimit-Limit", fmt.Sprint(response.Headers.XRateLimitLimit))
	w.Header().Set("X-RateLimit-Remaining", fmt.Sprint(response.Headers.XRateLimitRemaining))
...
type CreateRole404JSONResponse struct{ NotFoundJSONResponse }

func (response CreateRole404JSONResponse) VisitCreateRoleResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(404)
...
type CreateRole429JSONResponse struct{ RateLimitedJSONResponse }

func (response CreateRole429JSONResponse) VisitCreateRoleResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Retry-After", fmt.Sprint(response.Headers.RetryAfter))
	w.Header().Set("X-RateLimit-Limit", fmt.Sprint(response.Headers.XRateLimitLimit))
	w.Header().Set("X-RateLimit-Remaining", fmt.Sprint(response.Headers.XRateLimitRemaining))
...
type CreateRole500JSONResponse struct{ InternalErrorJSONResponse }
...
type DeleteRole429JSONResponse struct{ RateLimitedJSONResponse }

func (response DeleteRole429JSONResponse) VisitDeleteRoleResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Retry-After", fmt.Sprint(response.Headers.RetryAfter))
	w.Header().Set("X-RateLimit-Limit", fmt.Sprint(response.Headers.XRateLimitLimit))
	w.Header().Set("X-RateLimit-Remaining", fmt.Sprint(response.Headers.XRateLimitRemaining))
...
type DeleteRole500JSONResponse struct{ InternalErrorJSONResponse }
...
type GetRole429JSONResponse struct{ RateLimitedJSONResponse }

func (response GetRole429JSONResponse) VisitGetRoleResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Retry-After", fmt.Sprint(response.Headers.RetryAfter))
	w.Header().Set("X-RateLimit-Limit", fmt.Sprint(response.Headers.XRateLimitLimit))
	w.Header().Set("X-RateLimit-Remaining", fmt.Sprint(response.Headers.XRateLimitRemaining))
...
type UpdateRole429JSONResponse struct{ RateLimitedJSONResponse }

func (response UpdateRole429JSONResponse) VisitUpdateRoleResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Retry-After", fmt.Sprint(response.Headers.RetryAfter))
	w.Header().Set("X-RateLimit-Limit", fmt.Sprint(response.Headers.XRateLimitLimit))
	w.Header().Set("X-RateLimit-Remaining", fmt.Sprint(response.Headers.XRateLimitRemaining))
...
type StrictServerInterface interface {
	...
	// Make one of this organization's users an owner of a received grant
	// (POST /v1/organizations/{org_id}/project-grants/{grant_id}/owners)
	AssignProjectGrantOwner(ctx context.Context, request AssignProjectGrantOwnerRequestObject) (AssignProjectGrantOwnerResponseObject, error)
	// Remove an owner of a received grant
	// (DELETE /v1/organizations/{org_id}/project-grants/{grant_id}/owners/{user_id})
	RemoveProjectGrantOwner(ctx context.Context, request RemoveProjectGrantOwnerRequestObject) (RemoveProjectGrantOwnerResponseObject, error)
	// List the roles assigned through a received grant
	// (GET /v1/organizations/{org_id}/project-grants/{grant_id}/user-grants)
	...
	// Read a grant
	// (GET /v1/organizations/{org_id}/projects/{project_id}/grants/{grant_id})
	GetProjectGrant(ctx context.Context, request GetProjectGrantRequestObject) (GetProjectGrantResponseObject, error)
	// List a project's roles
	// (GET /v1/organizations/{org_id}/projects/{project_id}/roles)
...
// ListProjectGrantOwners operation middleware
func (sh *strictHandler) ListProjectGrantOwners(w http.ResponseWriter, r *http.Request, orgId OrganizationId, grantId ProjectGrantId) {
	var request ListProjectGrantOwnersRequestObject

	request.OrgId = orgId
	request.GrantId = grantId

	handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, request interface{}) (interface{}, error) {
		return sh.ssi.ListProjectGrantOwners(ctx, request.(ListProjectGrantOwnersRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "ListProjectGrantOwners")
	}
	...
	if err != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, err)
	} else if validResponse, ok := response.(ListProjectGrantOwnersResponseObject); ok {
		if err := validResponse.VisitListProjectGrantOwnersResponse(w); err != nil {
			sh.options.ResponseErrorHandlerFunc(w, r, err)
		}
	} else if response != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, fmt.Errorf("unexpected response type: %T", response))
...
// AssignProjectGrantOwner operation middleware
func (sh *strictHandler) AssignProjectGrantOwner(w http.ResponseWriter, r *http.Request, orgId OrganizationId, grantId ProjectGrantId, params AssignProjectGrantOwnerParams) {
	var request AssignProjectGrantOwnerRequestObject

	request.OrgId = orgId
	request.GrantId = grantId
	request.Params = params

	var body AssignProjectGrantOwnerJSONRequestBody
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		sh.options.RequestErrorHandlerFunc(w, r, fmt.Errorf("can't decode JSON body: %w", err))
		return
	}
	...
	handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, request interface{}) (interface{}, error) {
		return sh.ssi.AssignProjectGrantOwner(ctx, request.(AssignProjectGrantOwnerRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "AssignProjectGrantOwner")
	}
	...
	if err != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, err)
	} else if validResponse, ok := response.(AssignProjectGrantOwnerResponseObject); ok {
		if err := validResponse.VisitAssignProjectGrantOwnerResponse(w); err != nil {
			sh.options.ResponseErrorHandlerFunc(w, r, err)
		}
	} else if response != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, fmt.Errorf("unexpected response type: %T", response))
...
// RemoveProjectGrantOwner operation middleware
func (sh *strictHandler) RemoveProjectGrantOwner(w http.ResponseWriter, r *http.Request, orgId OrganizationId, grantId ProjectGrantId, userId UserId, params RemoveProjectGrantOwnerParams) {
	var request RemoveProjectGrantOwnerRequestObject
	...
	handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, request interface{}) (interface{}, error) {
		return sh.ssi.RemoveProjectGrantOwner(ctx, request.(RemoveProjectGrantOwnerRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "RemoveProjectGrantOwner")
	}
	...
	if err != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, err)
	} else if validResponse, ok := response.(RemoveProjectGrantOwnerResponseObject); ok {
		if err := validResponse.VisitRemoveProjectGrantOwnerResponse(w); err != nil {
			sh.options.ResponseErrorHandlerFunc(w, r, err)
		}
	} else if response != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, fmt.Errorf("unexpected response type: %T", response))
...
// ListProjectGrants operation middleware
func (sh *strictHandler) ListProjectGrants(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, params ListProjectGrantsParams) {
...
// CreateProjectGrant operation middleware
func (sh *strictHandler) CreateProjectGrant(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, params CreateProjectGrantParams) {
...
// RevokeProjectGrant operation middleware
func (sh *strictHandler) RevokeProjectGrant(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, grantId ProjectGrantId, params RevokeProjectGrantParams) {
...
// GetProjectGrant operation middleware
func (sh *strictHandler) GetProjectGrant(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, grantId ProjectGrantId) {
	var request GetProjectGrantRequestObject
	...
	handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, request interface{}) (interface{}, error) {
		return sh.ssi.GetProjectGrant(ctx, request.(GetProjectGrantRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "GetProjectGrant")
	}
	...
	if err != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, err)
	} else if validResponse, ok := response.(GetProjectGrantResponseObject); ok {
		if err := validResponse.VisitGetProjectGrantResponse(w); err != nil {
			sh.options.ResponseErrorHandlerFunc(w, r, err)
		}
	} else if response != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, fmt.Errorf("unexpected response type: %T", response))
...
// ListRoles operation middleware
func (sh *strictHandler) ListRoles(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, params ListRolesParams) {
...
// CreateRole operation middleware
func (sh *strictHandler) CreateRole(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, params CreateRoleParams) {
...
// DeleteRole operation middleware
func (sh *strictHandler) DeleteRole(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, roleId RoleId) {
...
// GetRole operation middleware
func (sh *strictHandler) GetRole(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, roleId RoleId) {
...
// UpdateRole operation middleware
func (sh *strictHandler) UpdateRole(w http.ResponseWriter, r *http.Request, orgId OrganizationId, projectId ProjectId, roleId RoleId) {
