...
**Why this phase is not optional**: every prior phase proved that features work. This phase proves the system holds under attack, under load, and under failure. `docs/PLAN/09-SECURITY.md` opens with the reason: "a compromise here means a compromise of *every* application that depends on it."

**Prerequisite**: Phase 4 exit checklist satisfied. Phase 4b either completed or explicitly declined in `MEMORY/DECISIONS.md`.

**Roadmap reference**: `docs/PLAN/16-IMPLEMENTATION-ROADMAP.md` § Phase 5.
...
| P5-04 | Load testing against `docs/PLAN/12` targets | backend, infra | L | Phase 4 exit |
...
| **Plan refs** | `docs/SECURITY/05-VERIFICATION-AND-REDTEAM-PLAN.md`, `docs/SECURITY/02-ATTACK-SURFACE-AND-SCENARIOS.md` (all 19 categories), `docs/PLAN/10-THREAT-MODEL.md` |
| **Spec required** | No |
| **Surface** | backend |

**Goal** — Walk every one of `docs/SECURITY/02`'s nineteen attack categories against the built system and confirm each has both a mitigation and a test — before paying an external firm to find what an afternoon of internal review would have caught.

**Steps**
1. Work through `docs/SECURITY/02` category by category: authentication attacks, authorization bypass/IDOR, privilege escalation including delegation abuse, session attacks, CSRF, XSS, SSRF, injection, file upload abuse, API abuse and rate-limit bypass, business-logic abuse, enumeration, credential stuffing and token leakage, insecure direct object access, supply-chain, secret exposure, container/runtime, CI/CD, and audit integrity.
2. For each, record: the implemented mitigation, the test that proves it, and the detection that would catch an attempt.
3. Follow `docs/SECURITY/05`'s verification approach per threat category and its "Definition of Verified."
4. Confirm every high-priority abuse scenario in `docs/PLAN/10` has an automated test — this is a `docs/PLAN/11` production-ready criterion.
...
8. Review the container and Kubernetes configuration against `docs/SECURITY/02` §17.

**Definition of Done**
- [ ] All nineteen `docs/SECURITY/02` categories have a documented mitigation, test, and detection.
- [ ] Every `docs/PLAN/10` high-priority abuse scenario has a passing automated test.
...
- [ ] Container and orchestration configuration is reviewed against `docs/SECURITY/02` §17.
- [ ] Findings are logged in `docs/PLAN/18-RISK-REGISTER.md` — via the deliberate-edit process, since it is a plan document.
...
| **Status** | TODO |
| **Depends on** | P5-01 |
| **Plan refs** | `docs/PLAN/09-SECURITY.md` § Secure Development Practices, `docs/SECURITY/05-VERIFICATION-AND-REDTEAM-PLAN.md` § External Penetration Testing, `docs/PLAN/16-IMPLEMENTATION-ROADMAP.md` § Phase 5 |
| **Spec required** | No |
| **Surface** | all |
...
**Steps**
1. Scope per `docs/SECURITY/05`: the OIDC and SAML surfaces, the Management API, the console, multi-tenant isolation, and the delegation model.
...
4. Direct explicit attention at the areas this project's own documents flag as highest risk: cross-tenant isolation (`docs/PLAN/08` Part B), delegation subset validation (`docs/PLAN/08` Part C), token handling, and session management.
...
- [ ] Scope covered every area named in `docs/SECURITY/05`.
...
| **Status** | TODO |
| **Depends on** | P5-02 |
| **Plan refs** | `docs/PLAN/17-ACCEPTANCE-CRITERIA.md` § Phase 5, `docs/PLAN/18-RISK-REGISTER.md`, `docs/PLAN/11-TESTING.md` |
| **Spec required** | Depends on findings |
| **Surface** | all |

**Goal** — Every critical and high finding either remediated or carrying a documented, accepted-risk sign-off — the exact wording of `docs/PLAN/17`'s Phase 5 criterion.

**Steps**
1. Triage every finding by severity and exploitability.
2. Remediate all critical and high findings.
3. For anything not remediated, record an explicit accepted-risk sign-off in `docs/PLAN/18-RISK-REGISTER.md` with owner, rationale, compensating controls, and a review date. "Accepted risk" without an owner and a date is just an unfixed bug with better vocabulary.
...
6. Feed structural findings back into `docs/SECURITY/02` and `docs/PLAN/10` — through the deliberate plan-edit process, since those are reference documents.

**Definition of Done**
- [ ] Every critical and high finding is remediated or has a signed-off accepted risk in `docs/PLAN/18`.
...
## P5-04 — Load Testing Against `docs/PLAN/12` Targets

| | |
|---|---|
| **Status** | TODO |
| **Depends on** | Phase 4 exit |
| **Plan refs** | `docs/PLAN/12-PERFORMANCE.md` § Latency Targets, § Load Testing Plan, `docs/PLAN/17-ACCEPTANCE-CRITERIA.md` § Phase 5 |
| **Spec required** | No |
| **Surface** | backend, infra |

**Goal** — Meet every latency target in `docs/PLAN/12`'s table under realistic load, measured rather than assumed.
...
2. Run the **mixed workload** test `docs/PLAN/12` requires: token issuance, management API traffic, and authz checks simultaneously — production never sees one traffic type in isolation.
...
4. Model realistic concurrency from expected consumer application behavior; `docs/PLAN/12` notes authz checks are usually the highest-volume traffic, since they happen on every protected request across every consumer app.
...
**Definition of Done**
- [ ] Every latency target in `docs/PLAN/12`'s table is met, or the gap is documented and explicitly accepted.
...
| **Status** | TODO |
| **Depends on** | P5-04 |
| **Plan refs** | `docs/PLAN/12-PERFORMANCE.md` § Load Testing Plan, `docs/PLAN/13-OBSERVABILITY.md` § Graceful Degradation Behavior |
| **Spec required** | No |
| **Surface** | backend, infra |
...
**Steps**
1. Test the scenarios `docs/PLAN/12` names explicitly: Redis slow, database read replica lagging, OPA evaluation artificially slowed.
2. Verify circuit breakers around Redis and database calls behave as `docs/PLAN/13` describes — a slow dependency must not block a request indefinitely.
3. Verify the fail-safe default: an authorization check that cannot complete **denies**. `docs/PLAN/13` is unambiguous that availability of a decision is never a reason to weaken security posture.
...
**Definition of Done**
- [ ] Every scenario in `docs/PLAN/12`'s degraded-dependency list is tested.
...
| **Status** | TODO |
| **Depends on** | P5-04 |
| **Plan refs** | `docs/PLAN/12-PERFORMANCE.md` § Definition of "Performance Acceptable", `docs/PLAN/14-DEPLOYMENT.md` § Scalability |
| **Spec required** | No |
| **Surface** | infra |

**Goal** — Verify horizontal scaling in practice — `docs/PLAN/12` says explicitly that this must be demonstrated by an actual scale-out test, "not just assumed from 'the service is stateless.'"
...
3. Verify the horizontal pod autoscaler responds to the intended signal (CPU or latency per `docs/PLAN/14`) with sensible thresholds.
...
6. Test Redis under session volume, and confirm whether cluster mode is needed (`docs/PLAN/14`).
...
| **Status** | TODO |
| **Depends on** | P0-20 |
| **Plan refs** | `docs/PLAN/15-DISASTER-RECOVERY.md`, `docs/PLAN/17-ACCEPTANCE-CRITERIA.md` § Phase 5 |
| **Spec required** | No |
| **Surface** | infra |

**Goal** — Execute a real drill, satisfying `docs/PLAN/17`'s criterion that one has been performed successfully within the last twelve months.

**Steps**
1. Follow `docs/PLAN/15`'s documented procedures rather than improvising — a drill that deviates from the runbook tests the team, not the runbook.
...
**Definition of Done**
- [ ] A full drill has been executed following `docs/PLAN/15`.
...
| **Plan refs** | `docs/SECURITY/03-DETECTION-AND-MONITORING.md`, `docs/PLAN/13-OBSERVABILITY.md` § Alerting, `docs/PLAN/09-SECURITY.md` § Audit |
| **Spec required** | No |
| **Surface** | backend, infra |

**Goal** — Every detection `docs/SECURITY/03` specifies is implemented, firing correctly, and routed to someone who will act on it.

**Steps**
1. Implement every detection in `docs/SECURITY/03`, and confirm coverage against the attack scenarios in `docs/SECURITY/02`.
2. Implement `docs/PLAN/13`'s named critical alerts: failed-login spikes from one IP or account, `/oauth/token` error rate above threshold, database replica lag, and sustained `/v1/authz/check` latency breach.
3. Forward the audit log to an external SIEM — `docs/PLAN/09` calls this "ideally," and by production launch it should be actual. `P0-12` left the hook.
...
**Definition of Done**
- [ ] Every `docs/SECURITY/03` detection is implemented and verified by induced condition.
- [ ] Every `docs/PLAN/13` critical alert fires correctly.
...
| **Status** | TODO |
| **Depends on** | P5-08 |
| **Plan refs** | `docs/SECURITY/04-INCIDENT-RESPONSE-PLAYBOOKS.md`, `docs/PLAN/15-DISASTER-RECOVERY.md` |
| **Spec required** | No |
| **Surface** | all |
...
**Steps**
1. Run a tabletop exercise for each playbook in `docs/SECURITY/04`.
...
**Definition of Done**
- [ ] Every `docs/SECURITY/04` playbook has been rehearsed.
...
| **Status** | TODO |
| **Depends on** | P1-24 |
| **Plan refs** | `docs/PLAN/16-IMPLEMENTATION-ROADMAP.md` § Phase 5, `docs/UI-UX/08-PAGE-SPECIFICATIONS.md` (Audit Log) |
| **Spec required** | No |
| **Surface** | console |
...
| **Plan refs** | `docs/UI-UX/13-ACCESSIBILITY.md`, `docs/UI-UX/17-UX-ACCEPTANCE-CRITERIA.md`, `docs/PLAN/02-REQUIREMENTS.md` (WCAG 2.1 AA), `docs/PLAN/17-ACCEPTANCE-CRITERIA.md` § Phase 5 |
| **Spec required** | No |
| **Surface** | console |

**Goal** — Pass a WCAG 2.1 AA audit — a Phase 5 acceptance criterion and a stated non-functional requirement since `docs/PLAN/02`.

**Steps**
1. Audit every screen against `docs/UI-UX/17-UX-ACCEPTANCE-CRITERIA.md`'s detailed checklist.
...
5. Verify color contrast for every token pair in every state (`docs/UI-UX/05` requires AA against both background tokens).
...
| **Status** | TODO |
| **Depends on** | all console tasks |
| **Plan refs** | `docs/PLAN/16-IMPLEMENTATION-ROADMAP.md` § Phase 5 ("UI-side load/perf pass"), `docs/UI-UX/12-RESPONSIVE-BEHAVIOR.md`, `docs/UI-UX/14-EMPTY-LOADING-ERROR-STATES.md` |
| **Spec required** | No |
| **Surface** | console |
...
4. Eliminate layout shift during loading, per `docs/UI-UX/14`'s loading-state requirements.
...
| **Status** | TODO |
| **Depends on** | P5-03 |
| **Plan refs** | `docs/PLAN/20-PUBLIC-SITE-ARCHITECTURE.md` § What Never Gets Published, `docs/UI-UX/20-PUBLIC-SITE-SPECIFICATIONS.md`, `docs/UI-UX/21-CONTENT-AND-COPY-STRATEGY.md` |
| **Spec required** | No |
| **Surface** | public-site |

**Goal** — Publish the trust page `docs/PLAN/20` schedules for this phase, communicating security posture without publishing defensive documentation.
...
2. Honor `docs/PLAN/20`'s explicit prohibitions absolutely. **Never publish**: specific attack scenarios or mitigation mechanics from `docs/SECURITY/02`; infrastructure topology from `docs/PLAN/14`; anything from `docs/PLAN/18-RISK-REGISTER.md`.
...
6. Apply `docs/UI-UX/21`'s governance rule: no claim beyond what actually ships.

**Definition of Done**
- [ ] The page is published and matches `docs/UI-UX/20`'s specification.
- [ ] A review confirms nothing from `docs/SECURITY/02`, `docs/PLAN/14` topology, or `docs/PLAN/18` appears on it.
...
| **Status** | TODO |
| **Depends on** | Phase 4 exit |
| **Plan refs** | `docs/PLAN/16-IMPLEMENTATION-ROADMAP.md` § Phase 5, `docs/PLAN/20-PUBLIC-SITE-ARCHITECTURE.md`, `docs/UI-UX/21-CONTENT-AND-COPY-STRATEGY.md` |
| **Spec required** | No |
| **Surface** | docs |

**Goal** — Complete documentation for internal and external developers who will integrate, per `docs/PLAN/16`'s Phase 5 item.

**Steps**
1. Complete the guide set for every shipped capability, verifying each by having someone unfamiliar follow it.
2. Complete the `/docs/console` section — how to use the management console at an end-user level (`docs/PLAN/20`).
3. Add a migration guide for teams moving off an existing auth system, honoring `docs/PLAN/01`'s constraint that migration is gradual and app-by-app, never a big-bang cutover.
...
- [ ] The migration guide reflects `docs/PLAN/01`'s gradual approach.
...
| **Status** | TODO |
| **Depends on** | P5-03 |
| **Plan refs** | `docs/PLAN/18-RISK-REGISTER.md`, `docs/PLAN/17-ACCEPTANCE-CRITERIA.md` § Phase 5, `AGENTS.md` rule 9 |
| **Spec required** | No |
| **Surface** | docs |
...
**Steps**
1. Review every risk in `docs/PLAN/18` against the built system: still relevant, mitigated, or superseded?
...
5. Because `docs/PLAN/18` is a reference document (`AGENTS.md` rule 9), make this a deliberate, reviewed edit rather than an incidental one.
...
| **Plan refs** | `docs/PLAN/17-ACCEPTANCE-CRITERIA.md` (all phases), `docs/PLAN/11-TESTING.md` § "Production-Ready" Criteria, `docs/PLAN/12-PERFORMANCE.md` § Definition of "Performance Acceptable" |
| **Spec required** | No |
| **Surface** | all |
...
**Steps**
1. Walk `docs/PLAN/17`'s criteria for every phase, confirming each still holds — a Phase 1 criterion can regress during Phase 4.
2. Confirm `docs/PLAN/11`'s "Production-Ready" criteria: every Phase 1 and 2 flow has passing integration and E2E tests in CI; no unaddressed critical or high SAST or dependency findings; load testing meets `docs/PLAN/12`; pentest findings reviewed and critical ones remediated; every high-priority abuse scenario has an automated test.
3. Confirm `docs/PLAN/12`'s "Performance Acceptable" definition: latency targets met, graceful degradation demonstrated, horizontal scaling verified in practice.
4. Confirm operational readiness: monitoring, alerting, on-call routing, runbooks, and rehearsed playbooks.
5. Confirm the general definition of done from `docs/PLAN/17` holds across the codebase — test coverage, OpenAPI accuracy, audit coverage, trust boundary analysis.
...
**Definition of Done**
- [ ] Every `docs/PLAN/17` criterion across every phase is verified as still holding.
- [ ] `docs/PLAN/11`'s production-ready criteria are all met.
- [ ] `docs/PLAN/12`'s performance-acceptable definition is met.
...
From `docs/PLAN/17-ACCEPTANCE-CRITERIA.md` § Phase 5:

- [ ] External pentest findings rated critical or high are all remediated, or have a documented, accepted-risk sign-off in `docs/PLAN/18-RISK-REGISTER.md`.
- [ ] Load test results meet every target in `docs/PLAN/12-PERFORMANCE.md`.
...
- [ ] All nineteen `docs/SECURITY/02` categories have documented mitigation, test, and detection.
