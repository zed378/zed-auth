...
**Roadmap reference**: `PLAN/16-IMPLEMENTATION-ROADMAP.md` § Phase 2. **Authorization source of truth**: `PLAN/08-AUTHORIZATION.md` Parts A and B.
...
| **Status** | TODO |
| **Depends on** | P1-17 |
| **Plan refs** | `PLAN/08-AUTHORIZATION.md` Part A, `PLAN/04-DATA-MODEL.md` § `roles` |
...
| **Status** | TODO |
| **Depends on** | P2-01 |
| **Plan refs** | `PLAN/05-API-CONTRACT.md` § Endpoint Structure, `UI-UX/08-PAGE-SPECIFICATIONS.md` (Roles tab) |
| **Spec required** | No |
| **Surface** | backend |
...
2. Require `PROJECT_OWNER` or higher (`PLAN/08` Part C hierarchy) — an ordinary org member must not be able to mint roles.
3. Validate permission keys on write, returning per-field errors in `PLAN/05`'s format.
4. Return the count of grants referencing each role in list responses, so the console can warn before deletion.
5. Register in the OpenAPI spec; the public API reference regenerates automatically (`PLAN/20`).
...
| **Plan refs** | `PLAN/04-DATA-MODEL.md` § `user_grants`, `PLAN/08-AUTHORIZATION.md` Part A, § Least Privilege |
...
**Steps**
1. Implement `user_grants` per `PLAN/04`: `user_id`, `project_id`, `project_grant_id` (nullable, unused until Phase 4), and `role_keys[]`.
2. Enforce the default from `PLAN/08` § Least Privilege: a new user has **no access** until explicitly granted. There is no implicit or default role.
...
6. Require re-authentication or MFA step-up for sensitive grant changes (`PLAN/08` § Least Privilege recommends this; decide and record whether Phase 2 or Phase 3 delivers it, since MFA itself is Phase 3).
...
**Definition of Done**
- [ ] A user with no grant has zero access, verified by test — this is a literal `PLAN/17` Phase 2 criterion.
...
**Abuse cases to test**
- Self-granting: a user assigning themselves a role they cannot administer (`SECURITY/02` §3).
- Assigning a role from a project the caller does not administer (`SECURITY/02` §2).
- Mass assignment of `role_keys` through an unrelated endpoint's body (`SECURITY/02` §11).
...
| **Plan refs** | `PLAN/08-AUTHORIZATION.md` Part A § How Role Claims Get Into the Token, Part C § Token Claim Format |
| **Spec required** | Yes — token format, hard to change later |
| **Surface** | backend |

**Goal** — Emit the claim format `PLAN/08` specifies exactly, including the `org_id` nested inside each role value — because that detail is what prevents ambiguity once Phase 4's delegation makes the same role name reachable from two organizational contexts.
...
1. Emit claims in the documented shape:
   ...
   ```
2. Include `org_id` inside the value even though Phase 2 has only one meaningful context — `PLAN/08` states the reason plainly, and adding it later is a breaking change for every consumer.
3. Emit `urn:authservice:manager_roles` for administrative roles (`PLAN/08` Part C).
...
6. Document in the public docs that claims are a **point-in-time snapshot**: a revocation after issuance is not reflected until expiry, which is exactly why `PLAN/08` says to prefer `/v1/authz/check` for sensitive actions.

**Definition of Done**
- [ ] The claim format matches `PLAN/08`'s documented JSON exactly, asserted by test.
- [ ] `org_id` is present inside every role value.
- [ ] A role assigned in Project A appears only in the Project A claim, scoped correctly — a literal `PLAN/17` Phase 2 criterion.
...
**Abuse cases to test**
- Token tampering to add a role, defeated by signature verification (`SECURITY/02` §1).
- Claim injection through a role key or display name containing JSON metacharacters.
- A role claim from org A honored while acting in org B's context (`PLAN/08` Part C, step 3a of the permission check flow).
...
| **Status** | TODO |
| **Depends on** | P1-15 |
| **Plan refs** | `PLAN/08-AUTHORIZATION.md` Part C § Manager Role Hierarchy & Inheritance, `PLAN/04-DATA-MODEL.md` § `manager_roles` |
| **Spec required** | Yes — administrative authorization |
| **Surface** | backend |

**Goal** — The full administrative hierarchy from `PLAN/08`, with inheritance flowing strictly downward and never upward.
...
2. Implement downward inheritance exactly as `PLAN/08` states: an `INSTANCE_OWNER` holds every `ORG_OWNER` right in every organization; the reverse never holds.
3. Encode the specific carve-out `PLAN/08` names: `ORG_ADMIN` has org access **except** deleting the organization or changing its owner.
4. Scope each manager role by `scope_id` (`PLAN/04`) — a `PROJECT_OWNER` on project X is not one on project Y.
...
6. Require confirmation and audit for granting `INSTANCE_OWNER` or `ORG_OWNER` — these are the keys to the system (`PLAN/08` § Least Privilege).
7. Guard against removing the last `ORG_OWNER` from an organization, which would orphan it.

**Definition of Done**
- [ ] The hierarchy matches `PLAN/08`'s diagram, with an exhaustive table-driven test over every (role, scope, action) combination.
...
**Abuse cases to test**
- Horizontal escalation: an `ORG_ADMIN` in org A acting in org B (`SECURITY/02` §3).
...
| **Status** | TODO |
| **Depends on** | P2-03 |
| **Plan refs** | `PLAN/05-API-CONTRACT.md` § Authorization Check Endpoint, `PLAN/08-AUTHORIZATION.md` Part A, `PLAN/12-PERFORMANCE.md`, `PLAN/13-OBSERVABILITY.md` |
| **Spec required** | Yes — authorization core |
| **Surface** | backend |
...
**Steps**
1. Implement the documented contract from `PLAN/05`: `subject`, `action`, `resource`, `context` in; `allowed`, `matched_policy`, `reasons` out.
...
5. **Never log the raw `resource.attributes`** — `PLAN/13` and `CLAUDE.md` both name this explicitly, and the attributes may contain the consumer application's business data.
6. Fail closed: a check that cannot complete because a dependency is down denies (`PLAN/13` § Graceful Degradation — "availability of a decision is never a reason to weaken security posture").
7. Meet `PLAN/12`'s targets: p50 < 20ms, p95 < 80ms, p99 < 150ms for RBAC-only checks.
8. Instrument decision latency, allow/deny ratio, and error rate, feeding `PLAN/13`'s alert on sustained latency breach.

**Definition of Done**
- [ ] Request and response match `PLAN/05`'s documented example exactly.
...
- [ ] `PLAN/12`'s RBAC latency targets are met under load.
- [ ] `reasons` is populated and useful for support and audit.

**Abuse cases to test**
- Querying a decision about a subject the caller has no business asking about (`SECURITY/02` §2).
- Using the endpoint as an enumeration oracle to map another organization's users or resources (`SECURITY/02` §12).
...
| **Status** | TODO |
| **Depends on** | P2-06 |
| **Plan refs** | `PLAN/12-PERFORMANCE.md` § Design Decisions Made for Performance, `PLAN/08-AUTHORIZATION.md` Part C § Full Permission Check Flow |
| **Spec required** | Yes — security/performance trade-off |
| **Surface** | backend |

**Goal** — Meet the latency target without letting a revocation linger. `PLAN/08` and `PLAN/12` both prescribe short-TTL caching specifically so a DB round-trip isn't needed per check while revocations are still caught quickly.
...
| **Plan refs** | `PLAN/08-AUTHORIZATION.md` Part B, `PLAN/02-REQUIREMENTS.md` FR-11, `PLAN/01-PRODUCT-SCOPE.md` § Out of Scope |
| **Spec required** | Yes — tenant isolation |
| **Surface** | backend |

**Goal** — Turn on the multi-tenancy the schema has carried since Phase 0, without the migration `PLAN/02`'s assumption promised would not be needed.

**Steps**
1. Verify the promise: a second organization can be created and used with **no schema migration** (`PLAN/02` § Assumptions). If a migration turns out to be needed, that is a plan deviation requiring an ADR.
...
5. Keep the deferral from `PLAN/01` intact: one user belonging to multiple organizations simultaneously is **out of scope**. Do not build workspace switching for a single identity; `P2-13`'s switcher is for an admin who holds manager roles in more than one org, which is a different thing.
6. Verify the audit log is org-scoped on read and that no filter combination crosses tenants.
7. Load-test with multiple organizations present to confirm RLS does not degrade query plans (`PLAN/08` Part B mentions per-`org_id` indexing and partitioning as the growth strategy).
...
**Abuse cases to test**
- Cross-tenant IDOR on every resource type (`SECURITY/02` §2, §14).
- Tenant context confusion: switching context mid-request or mid-session.
- Enumerating another organization's existence through error differences (`SECURITY/02` §12).
...
| **Status** | TODO |
| **Depends on** | P2-08 |
| **Plan refs** | `PLAN/08-AUTHORIZATION.md` Part B § Tenant Resolution |
| **Spec required** | Yes — authentication routing |
| **Surface** | backend |

**Goal** — Decide and implement how a request's organization is determined, from the combinable options `PLAN/08` lists: subdomain, path, email domain at login, or a single default organization.
...
5. Support the single-default-organization mode for purely internal deployments (`PLAN/08` Part B), since that is what Phase 1 shipped with.
...
| **Plan refs** | `PLAN/08-AUTHORIZATION.md` Part B § Policies per Organization, `PLAN/02-REQUIREMENTS.md` FR-12, `PLAN/17-ACCEPTANCE-CRITERIA.md` § Phase 2 |
| **Spec required** | Yes — security policy |
| **Surface** | backend |

**Goal** — Organization settings that are **enforced at login time, not merely stored** — the precise wording of `PLAN/17`'s Phase 2 criterion.
...
| **Status** | TODO |
| **Depends on** | P2-02 |
| **Plan refs** | `UI-UX/08-PAGE-SPECIFICATIONS.md` (Roles tab), `UI-UX/19-FRONTEND-IMPLEMENTATION-CHAIN.md`, `UI-UX/15-FORM-UX.md` |
...
**Steps**
1. Run the full `UI-UX/19` implementation chain.
...
| **Status** | TODO |
| **Depends on** | P2-03 |
| **Plan refs** | `UI-UX/08-PAGE-SPECIFICATIONS.md` (Authorizations tab), `UI-UX/04-USER-FLOWS.md`, `UI-UX/06-VISUAL-LANGUAGE.md` (role-source badge) |
| **Spec required** | No — implementation chain mandatory |
| **Surface** | console |

**Goal** — Search for a user and assign or revoke project roles, with the role-source badge in place from the start — `UI-UX/08` makes that badge mandatory on every screen showing roles, not optional per screen.
...
| **Status** | TODO |
| **Depends on** | P2-08 |
| **Plan refs** | `UI-UX/08-PAGE-SPECIFICATIONS.md` (Organization switcher), `PLAN/06-FRONTEND-ARCHITECTURE.md` § Information Architecture, `PLAN/01-PRODUCT-SCOPE.md` § Out of Scope |
| **Spec required** | No |
| **Surface** | console |

**Goal** — Let an admin holding manager roles in more than one organization switch context — while respecting `PLAN/01`'s explicit deferral of one user belonging to multiple organizations as an end-user feature.

**Steps**
1. Show the switcher only when the signed-in admin actually administers more than one organization (`UI-UX/08`: relevant only once multi-org is active).
2. Search input plus list, per `UI-UX/08`'s component composition.
...
6. Verify server-side that the caller may act in the selected organization — the switcher is UI, not a control (`UI-UX/08` § Cross-Screen Requirements).
...
| **Status** | TODO |
| **Depends on** | P2-10 |
| **Plan refs** | `UI-UX/08-PAGE-SPECIFICATIONS.md` (Policies — Access tab), `UI-UX/15-FORM-UX.md`, `PLAN/08-AUTHORIZATION.md` Part B |
| **Spec required** | No — implementation chain mandatory |
| **Surface** | console |
...
**Definition of Done**
- [ ] Settings map exactly to `PLAN/08` Part B's documented JSON shape.
...
- [ ] Validation errors render inline per `UI-UX/15`.
- [ ] Accessibility and responsive requirements are met.
...
| **Plan refs** | `PLAN/20-PUBLIC-SITE-ARCHITECTURE.md` § Site Structure, `UI-UX/21-CONTENT-AND-COPY-STRATEGY.md` |
| **Spec required** | No |
| **Surface** | docs |
...
**Steps**
1. Concepts page for roles, permission keys, grants, and the manager role hierarchy, derived from `PLAN/08` Parts A and B at a product-explainer level.
...
| **Plan refs** | `PLAN/11-TESTING.md`, `PLAN/10-THREAT-MODEL.md`, `SECURITY/02` §2, §3 |
| **Spec required** | No |
...
2. **Integration**: the full flow from `PLAN/11` — organization → project → application → user → role assignment → login → verify token claims are correct and correctly scoped.
...
| **Status** | TODO |
| **Depends on** | P2-16 |
| **Plan refs** | `PLAN/17-ACCEPTANCE-CRITERIA.md` § Phase 2, `PLAN/09-SECURITY.md` § Secure Development Practices |
| **Spec required** | No |
| **Surface** | all |

**Goal** — Verify `PLAN/17`'s four Phase 2 criteria with evidence, and threat-model Phase 3 before starting it.
...
2. Load-test `/v1/authz/check` against `PLAN/12`'s RBAC targets.
3. Run the Phase 3 threat-model review (`PLAN/09`).
4. Write the phase summary in `MEMORY/`.
5. Update `PROGRESS.md`; tag the release; publish the changelog.

**Definition of Done**
- [ ] All four `PLAN/17` Phase 2 criteria verified with evidence.
...
From `PLAN/17-ACCEPTANCE-CRITERIA.md` § Phase 2:
...
- [ ] The token claim format matches `PLAN/08` exactly, including the nested `org_id`.
