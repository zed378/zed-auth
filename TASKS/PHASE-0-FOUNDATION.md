...
**Exit criteria**: `docker compose up` produces a service that answers `/healthz`, connects to Postgres and Redis, has the full schema from `docs/PLAN/04-DATA-MODEL.md` applied by migration, emits structured JSON logs with a request ID, and exposes Prometheus metrics — and a push to `main` runs build + test + SAST + dependency scan in CI. The public landing page, About page, and docs skeleton are live at a real URL.

**Roadmap reference**: `docs/PLAN/16-IMPLEMENTATION-ROADMAP.md` § Phase 0.
...
| **Status** | TODO |
| **Depends on** | — |
| **Plan refs** | `docs/PLAN/07-BACKEND-ARCHITECTURE.md`, `docs/PLAN/06-FRONTEND-ARCHITECTURE.md`, `docs/PLAN/20-PUBLIC-SITE-ARCHITECTURE.md` |
| **Spec required** | No |
| **Surface** | docs |

**Goal** — Convert `docs/PLAN/07`'s explicitly-labelled "initial recommendations" into decisions the rest of the project can build on without re-litigating.

**Steps**
1. Confirm Go as the backend language, or record the alternative — `docs/PLAN/07` names TypeScript/NestJS and Java/Spring Authorization Server as acceptable substitutes if the team's existing skills point elsewhere.
2. Choose the OAuth2/OIDC provider library: `ory/fosite` or `zitadel/oidc`. Evaluate against four questions that later phases depend on: does it support Authorization Code + PKCE with PKCE mandatory for confidential clients too (`docs/PLAN/05-API-CONTRACT.md`), pluggable storage, JWKS key rotation with an overlap window, and refresh token rotation with reuse detection (needed in Phase 3)?
...
4. Choose the migration tool (`golang-migrate`, `goose`, or `atlas`) — it must support running migrations as a **separate step before rollout**, never on service startup (`docs/PLAN/14-DEPLOYMENT.md`).
5. Choose the public-site stack: a static generator for marketing (Astro or Next static export) plus a docs framework with native versioning (`docs/PLAN/20` recommends Docusaurus).
...
2. Add `deploy/` for Docker Compose, Kubernetes manifests, and environment configuration — `docs/PLAN/14-DEPLOYMENT.md` needs a home.
3. Add `openapi/` for the specification that both the console client and the public API reference are generated from (`docs/PLAN/05`, `docs/PLAN/20`).
...
6. Leave `docs/PLAN/`, `docs/UI-UX/`, and `docs/SECURITY/` untouched — they are reference material (`AGENTS.md` rule 9).

**Definition of Done**
- [ ] The layout matches `CLAUDE.md` exactly, plus `deploy/` and `openapi/`.
- [ ] `.gitignore` prevents any `.env` file from ever being committed (`docs/SECURITY/02-ATTACK-SURFACE-AND-SCENARIOS.md` §16 Secret Exposure).
...
4. Add `CODEOWNERS` marking `docs/PLAN/`, `docs/UI-UX/`, and `docs/SECURITY/` as requiring explicit review — they should change only deliberately.
...
- [ ] Editing anything under `docs/PLAN/`, `docs/UI-UX/`, or `docs/SECURITY/` requires a code-owner review.
...
| **Status** | TODO |
| **Depends on** | P0-02 |
| **Plan refs** | `docs/PLAN/07-BACKEND-ARCHITECTURE.md` § Service Layout, `docs/PLAN/14-DEPLOYMENT.md` § Deployment Model |
| **Spec required** | No |
| **Surface** | backend |
...
2. Create the package layout reflecting `docs/PLAN/07`'s modular monolith:
   ```
   backend/
...
5. Graceful shutdown on `SIGTERM`: stop accepting connections, drain in-flight requests within a bounded timeout, then exit — required because this service sits on every consumer app's critical path (`docs/PLAN/14`).
...
| **Status** | TODO |
| **Depends on** | P0-04 |
| **Plan refs** | `docs/PLAN/14-DEPLOYMENT.md` § Environment Strategy, `docs/PLAN/13-OBSERVABILITY.md` § Environments |
| **Spec required** | No |
| **Surface** | infra |
...
2. A multi-stage `Dockerfile` for the backend producing a minimal non-root runtime image (`docs/SECURITY/02` §17 Container / Runtime Security).
3. Pin image versions — never `:latest` (supply-chain risk, `docs/SECURITY/02` §15).
...
| **Status** | TODO |
| **Depends on** | P0-05 |
| **Plan refs** | `docs/PLAN/14-DEPLOYMENT.md` § Release Process, § Rollback Strategy |
| **Spec required** | No |
| **Surface** | backend |
...
3. Document the expand/contract discipline in `backend/migrations/README.md`: every migration must leave the previous application version able to run against the new schema, so an app rollback never requires a DB rollback (`docs/PLAN/14`).
...
| **Status** | TODO |
| **Depends on** | P0-06 |
| **Plan refs** | `docs/PLAN/04-DATA-MODEL.md` (all of it), `docs/PLAN/08-AUTHORIZATION.md` Part B |
| **Spec required** | Yes — data model change |
| **Surface** | backend |

**Goal** — The complete entity model from `docs/PLAN/04` exists in Postgres, multi-tenant-ready from day one even though the MVP runs with a single default organization.
...
2. Later-phase tables may be created now or deferred to the phase that uses them — `user_mfa_factors` and `user_recovery_codes` (Phase 3), `user_identities`, `webhook_endpoints`, `webhook_deliveries` (Phase 4), `user_attributes` and `policies` (Phase 4b). Defer by default, per `docs/PLAN/00`'s principle of not over-engineering ahead of need; create early only where doing so costs nothing. Record the choice in the MEMORY record either way.
3. `signing_keys` and `user_tokens` are **not** deferrable — `P1-03` and `P1-19` both need them inside Phase 1.
4. Include the columns Phase 3 depends on but Phase 1 does not use: `refresh_tokens.family_id` and `replaced_by` (`docs/PLAN/04`). The storage shape must not need changing when rotation arrives.
5. Partition `events` by month on `created_at` from the start (`docs/PLAN/04` § Retention and Growth) — retrofitting partitioning onto a large table is far more disruptive than starting with it.
6. Enforce the constraints the plan states rather than implies:
   - `users.email` unique **per org**, not globally (`docs/PLAN/04`) — a composite unique index on `(org_id, lower(email))`.
   ...
   - `user_identities` unique on `(provider, provider_subject)` — matching is never on email (`docs/PLAN/04`).
   - `roles` unique on `(project_id, key)`.
7. Put `org_id` on every tenant-scoped table, even where it is derivable through a join — RLS in `P0-08` needs it directly, and a misscoped query is exactly the failure mode `docs/PLAN/08` Part B guards against. This now includes `sessions.org_id` (`docs/PLAN/04`).
...
9. Make `events` append-only at the database level: revoke `UPDATE` and `DELETE` from the application role (`docs/SECURITY/02` §19 Logging / Audit Integrity).
...
**Definition of Done**
- [ ] Every Phase 1 table and column in `docs/PLAN/04-DATA-MODEL.md` exists with the stated nullability and types; every deferred table is named in the MEMORY record with the phase that will create it.
...
- [ ] An ERD generated from the live schema matches the diagram in `docs/PLAN/04`.
...
- Attempted tampering with an `events` row (`docs/SECURITY/02` §19).
...
| **Status** | TODO |
| **Depends on** | P0-07 |
| **Plan refs** | `docs/PLAN/08-AUTHORIZATION.md` Part B, `docs/SECURITY/02-ATTACK-SURFACE-AND-SCENARIOS.md` §2 |
| **Spec required** | Yes — authorization boundary |
| **Surface** | backend |

**Goal** — Cross-tenant isolation that holds even when the application layer forgets to filter. `docs/PLAN/08` explicitly wants "a misscoped query can't leak data across organizations" to be a database property, not a code-review outcome.
...
**Abuse cases to test**
- `docs/SECURITY/02` §2 Authorization Bypass / IDOR: request a resource ID belonging to another organization and confirm a response that does not confirm the resource's existence (see also §12 Enumeration).
...
| **Status** | TODO |
| **Depends on** | P0-04 |
| **Plan refs** | `docs/PLAN/13-OBSERVABILITY.md` § Logging, `docs/SECURITY/02` §16, `docs/PLAN/10-THREAT-MODEL.md` § Information Disclosure |
| **Spec required** | No |
| **Surface** | backend |
...
4. Set log levels per `docs/PLAN/13`: a failed login is `WARN`, not `ERROR` — it is expected behavior, and treating it as an error trains everyone to ignore errors.
...
| **Status** | TODO |
| **Depends on** | P0-04 |
| **Plan refs** | `docs/PLAN/14-DEPLOYMENT.md` § Deployment Model |
| **Spec required** | No |
| **Surface** | backend |
...
3. Neither endpoint discloses versions, hostnames, dependency addresses, or error detail to an unauthenticated caller (`docs/SECURITY/02` §12 Enumeration).
...
| **Status** | TODO |
| **Depends on** | P0-09 |
| **Plan refs** | `docs/PLAN/13-OBSERVABILITY.md` § Metrics, § Tracing, `docs/PLAN/12-PERFORMANCE.md` |
| **Spec required** | No |
| **Surface** | backend |
...
2. Baseline instruments matching what `docs/PLAN/13` requires: request latency histograms per endpoint (p50/p95/p99 derivable), error rate per endpoint, DB pool utilization, Redis operation latency.
...
5. A starter Grafana dashboard in `deploy/` whose panels map one-to-one onto `docs/PLAN/12`'s latency target table, so "are we meeting target" is answerable at a glance rather than by ad-hoc querying.
...
- [ ] The dashboard's panels correspond one-to-one with `docs/PLAN/12`'s target table rows.
...
| **Plan refs** | `docs/PLAN/04-DATA-MODEL.md` § `events`, `docs/PLAN/09-SECURITY.md` § Audit, `docs/SECURITY/02` §19 |
...
5. Provide a query helper with pagination for the Audit Log console screen (`docs/UI-UX/08-PAGE-SPECIFICATIONS.md`) and for Phase 5's export.
6. Include a SIEM forwarding hook as a no-op implementation now, so `docs/PLAN/09`'s "ideally forwarded to an external SIEM" has somewhere to plug in later.
...
| **Status** | TODO |
| **Depends on** | P0-04 |
| **Plan refs** | `docs/PLAN/14-DEPLOYMENT.md` § Release Process, `docs/PLAN/09-SECURITY.md` § Secure Development Practices, `docs/PLAN/11-TESTING.md` |
| **Spec required** | No |
| **Surface** | infra |
...
3. Fail the build on critical or high findings from SAST or dependency scanning — `docs/PLAN/11`'s "Production-Ready" criteria require no unaddressed critical/high.
4. Pin action versions by commit SHA rather than by tag: CI is itself an attack surface (`docs/SECURITY/02` §18 CI/CD Attack Surface).
...
6. Add a secret-scanning step over the diff, so a committed credential is caught at PR time (`docs/SECURITY/02` §16).
...
| **Status** | TODO |
| **Depends on** | P0-04 |
| **Plan refs** | `docs/PLAN/07-BACKEND-ARCHITECTURE.md` § Infrastructure, `docs/PLAN/09-SECURITY.md`, `docs/SECURITY/02` §16 |
| **Spec required** | No |
| **Surface** | infra |

**Goal** — Establish, before the first signing key exists, where secrets live and how they reach the service — because `docs/PLAN/02`'s constraint is that no third party ever holds the private signing key.
...
2. Enumerate every secret the system will hold, with owner and rotation expectation: JWT signing private keys, database credentials, Redis credentials, OIDC client secrets (hashed at rest per `docs/PLAN/04`), SMTP credentials, and later social-IdP client secrets.
3. Define separate secrets per environment — `docs/PLAN/13` and `docs/PLAN/14` both state that staging and production never share signing keys or databases.
...
**Goal** — All four layers of `docs/PLAN/11`'s pyramid have a working example test before Phase 1 starts, so no Phase 1 task can plead "there's no harness for that yet."
...
4. A security-test package kept separate from feature tests, so `docs/PLAN/11` § Security Testing scenarios stay visible as a group rather than scattered.
...
- [x] The security-test package exists with at least one real assertion. Four scenarios from `docs/PLAN/11` § Security Testing, plus a control test — and all of them fail when pointed at a role that bypasses RLS, which is how we know they measure the property.
...
| **Depends on** | P0-13 |
| **Plan refs** | `docs/PLAN/05-API-CONTRACT.md` § Documentation, `docs/PLAN/06-FRONTEND-ARCHITECTURE.md` § Tech Stack, `docs/PLAN/20-PUBLIC-SITE-ARCHITECTURE.md` § API Reference Generation |
| **Spec required** | No |
| **Surface** | backend, console |
...
**Steps**
1. Create `openapi/openapi.yaml` with info, servers, security schemes (OAuth2 bearer), and the shared component schemas the whole API depends on: the standard error object from `docs/PLAN/05` (`code`, `message`, `details[]`), the pagination envelope (`next_page_token`), and common parameters (`page_size`, `page_token`).
2. Decide spec-first versus code-first generation and record it as an ADR. Spec-first is easier to review; code-first is harder to let drift. `docs/PLAN/05` accepts either as long as CI validates the result.
3. Wire the console's typed API client generation, so a spec change regenerates client types.
4. Wire the public site's API reference rendering from the same file (`docs/PLAN/20`).
...
- [x] The error and pagination schemas match `docs/PLAN/05-API-CONTRACT.md` exactly.
...
| **Plan refs** | `docs/PLAN/06-FRONTEND-ARCHITECTURE.md`, `docs/UI-UX/05-DESIGN-SYSTEM.md`, `docs/UI-UX/06-VISUAL-LANGUAGE.md`, `docs/UI-UX/12-RESPONSIVE-BEHAVIOR.md`, `docs/UI-UX/13-ACCESSIBILITY.md` |
| **Spec required** | No |
| **Surface** | console |

**Goal** — A React + TypeScript application shell whose design tokens are implemented *as tokens*, so no screen ever hard-codes a color and the rebrand-safety property `docs/UI-UX/05` requires actually holds.

**Steps**
1. Scaffold React + TypeScript with the chosen build tool; static SPA output, deployable to a CDN independently of the backend (`docs/PLAN/06`).
2. Implement every token from `docs/UI-UX/05-DESIGN-SYSTEM.md` as CSS custom properties mapped into the Tailwind theme: the nine color tokens, the constrained type scale, the 4px/8px spacing scale, and the three elevation levels.
3. Enforce token-only usage with a lint rule rejecting raw hex colors and arbitrary spacing values in application code.
4. Encode the branding constraint structurally: `color-accent` and the logo are overridable per organization; `color-danger` and `color-warning` are not (`docs/UI-UX/05` § Color).
5. Application shell: routing, the navigation structure from `docs/PLAN/06`'s IA, the 12/8/4-column responsive grid from `docs/UI-UX/12`, and an error boundary.
6. TanStack Query for server state, with no global store for cached API data (`docs/PLAN/06`).
7. Wire the generated API client from `P0-16`.
8. Accessibility baseline from the start: visible focus rings using `color-accent`, a skip-to-content link, and correct landmark regions (`docs/UI-UX/13`). Retrofitting these in Phase 5 is how accessibility audits fail.

**Definition of Done**
- [x] Every token in `docs/UI-UX/05` exists in code, by name. Asserted by `tokens.test.ts`, which also fails if a tenth colour token appears — that would be a design-system change and `docs/UI-UX/05` § Governance requires it to happen there first.
...
- [x] The shell renders correctly at 1440px, 1024px, and 768px per `docs/UI-UX/12`'s grid. Measured in a real browser against `console.zedth.my.id`, plus 600px for `docs/UI-UX/12` § Testing's unsupported-width message.
...
| **Depends on** | P0-02 |
| **Plan refs** | `docs/PLAN/20-PUBLIC-SITE-ARCHITECTURE.md`, `docs/UI-UX/20-PUBLIC-SITE-SPECIFICATIONS.md` |
| **Spec required** | No |
| **Surface** | public-site |
...
2. Scaffold the docs section with the docs framework, versioning enabled from the start. Retrofitting versioning once v1 docs exist is painful, and `docs/PLAN/20` requires old-version docs to stay reachable through their deprecation window.
3. Build the route structure from `docs/PLAN/20`: `/`, `/about`, `/docs` (with `quickstart`, `concepts`, `guides`, `api-reference`, `console`), `/changelog`, `/security`, `/contact`. Omit `/pricing` entirely unless a commercial tier exists.
4. Share only the visual language with the console (`docs/UI-UX/06-VISUAL-LANGUAGE.md`) — brand-level tokens, not components, not code.
...
- [ ] **Lighthouse performance and SEO scores meet the bar set in `docs/UI-UX/20` § Cross-Page Requirements.** That section sets an accessibility bar and no numeric performance target, so there is no bar to meet. Raised as `OQ-10`; settle the number or drop the item.
...
| **Depends on** | P0-18 |
| **Plan refs** | `docs/UI-UX/20-PUBLIC-SITE-SPECIFICATIONS.md`, `docs/UI-UX/21-CONTENT-AND-COPY-STRATEGY.md`, `docs/PLAN/20-PUBLIC-SITE-ARCHITECTURE.md` § Deployment & Roadmap Placement |
| **Spec required** | No |
...
**Steps**
1. Build the landing page to `docs/UI-UX/20`'s detailed spec, using the copy blueprints in `docs/UI-UX/21`.
2. Build the About page.
3. Write `/docs/concepts` — organizations, projects, applications, users, roles, grants — at a product-explainer level derived from `docs/PLAN/03` and `docs/PLAN/08`. Concepts are safe to document now because they describe the model, not shipped endpoints.
...
5. Publish `/contact` with the responsible-disclosure channel (`security@` or a form), per `docs/PLAN/20`'s requirement that external researchers have a documented path.
6. Apply the governance rule as a review gate: no marketing or docs sentence claims a capability beyond the current phase (`docs/UI-UX/21`, `CLAUDE.md`).
7. Do **not** publish `/security` content yet — `docs/PLAN/20` places the trust page alongside Phase 5, and it must never leak `docs/SECURITY/` internals.

**Definition of Done**
- [x] Landing and About pages match `docs/UI-UX/20`'s spec and `docs/UI-UX/21`'s copy blueprints — with the capability copy converted from present tense to labelled-as-planned, since `docs/UI-UX/21`'s own governance section forbids what its blueprint literally says.
...
- [x] No content derived from `docs/SECURITY/02`, `docs/PLAN/14` topology, or `docs/PLAN/18` appears anywhere on the site. `check-no-internal-leak.mjs` scans 35 built pages against 3,240 verbatim phrases on every build.

**Beyond the stated steps**

Both audits were verified by breaking them rather than by passing: a sentence from `docs/SECURITY/02` pasted into `/about`, a private IP on `/contact`, and a capability card stripped of its phase label. The landing page also gained the navigation CTA `docs/UI-UX/20` § Above-the-fold requires, and its three primary-action labels were collapsed to one.
...
| **Depends on** | P0-13 |
| **Plan refs** | `docs/PLAN/14-DEPLOYMENT.md`, `docs/PLAN/13-OBSERVABILITY.md` § Environments, `docs/PLAN/15-DISASTER-RECOVERY.md` |
| **Spec required** | No |
| **Surface** | infra |
...
3. Terminate TLS at the ingress; no HTTP fallback anywhere (`docs/PLAN/09`).
4. Generate staging-only signing keys. Never copy production keys into staging, and never copy production data into staging (`docs/PLAN/13`, `docs/PLAN/14`).
...
6. Run migrations as a separate pipeline step before the application rollout (`docs/PLAN/14`).
7. Configure automated Postgres backups with point-in-time recovery, and verify a restore actually works — an unverified backup is a hypothesis (`docs/PLAN/15`).
