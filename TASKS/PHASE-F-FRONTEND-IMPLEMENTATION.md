...
**Goal**: every page and every component of the three frontend surfaces, specified in one place — the design system components from `docs/UI-UX/07-COMPONENT-SPECIFICATION.md`, all 21 console screens from `docs/UI-UX/08-PAGE-SPECIFICATIONS.md`, the hosted authentication screens, and the public site from `docs/UI-UX/20-PUBLIC-SITE-SPECIFICATIONS.md`.

---

## Read This First: How Phase F Relates to Phases 0–5

`docs/PLAN/16-IMPLEMENTATION-ROADMAP.md` says plainly:
...
**Relationship to the console tasks already in Phases 1–5.** Those tasks are not duplicates and are not superseded — they remain the **phase gates**, verifying the screens against that phase's acceptance criteria (`docs/PLAN/17`). Phase F holds the implementation detail. The mapping:
...
`docs/UI-UX/19-FRONTEND-IMPLEMENTATION-CHAIN.md` defines twelve steps every screen and component must be run through before it is implementable:
...
The chain exists because, without it, Loading/Error/Empty/Permission/Accessibility get invented ad hoc by whoever implements each screen — which is the specific inconsistency `docs/UI-UX/05` and `docs/UI-UX/14` exist to prevent.

---

## Console-Wide Rules Every Task Inherits

Pulled from across `docs/UI-UX/` so they are enforceable rather than remembered:

1. **Tokens only.** No raw hex, no arbitrary spacing. Any new token or component is added to the design system *before* a page uses it (`docs/UI-UX/05` § Governance).
2. **`color-danger` is reserved for destructive and irreversible actions.** Never for errors that are merely unwanted, never decoratively (`docs/UI-UX/06`, `CLAUDE.md`).
3. **Consequence before confirmation.** Access-changing actions compute and show the real consequence in plain language, and the confirm button carries the actual verb — never "OK" (`docs/UI-UX/09`).
4. **Confirm-before, not undo-after**, for anything touching access control. A brief window of wrong access may already have been exploited (`docs/UI-UX/09` § Undo vs. Confirm).
5. **Permission-gated routes are genuinely unreachable, not hidden.** The frontend checks the same claims the API enforces, and the API enforces independently regardless (`docs/UI-UX/08` § Cross-Screen Requirements, `CLAUDE.md`).
6. **Every table showing roles or grants carries the role-source badge.** Not optional per screen (`docs/UI-UX/07`, `docs/UI-UX/08`).
7. **Never color or icon alone.** Status always pairs color with icon *and* text (`docs/UI-UX/13`).
8. **Genuinely empty ≠ filtered to empty.** Different copy, different recovery action (`docs/UI-UX/14`).
9. **Skeletons, not blocking spinners**, and skeletons preserve final geometry so content never jumps (`docs/UI-UX/09`, `docs/UI-UX/14`).
10. **Hover is never the only interactivity cue** — tablet has no hover (`docs/UI-UX/09`).
11. **Every animation honors `prefers-reduced-motion`** with an instant fallback. Hard requirement (`docs/UI-UX/13`).
12. **Nothing in the UI claims a capability that isn't shipped in the current phase** (`docs/UI-UX/21`, `CLAUDE.md`).
...
| **Plan refs** | `docs/UI-UX/10-MOTION-DESIGN.md`, `docs/UI-UX/13-ACCESSIBILITY.md` § Motion & Animation |

**Goal** — Durations and easing curves as tokens, not hard-coded per component. `docs/UI-UX/10` § Ownership asks for exactly this "once implementation begins" — this is that moment.
...
4. Encode `docs/UI-UX/10`'s two "never gets motion" rules structurally: destructive confirmation dialogs appear instantly with no entrance animation, and table data never animates on sort or filter.
...
**Definition of Done**
- [ ] Every animation in `docs/UI-UX/10`'s table has a token-driven implementation.
...
|---|---|
| **Status** | TODO · **Depends on** P0-17 · **Surface** console · **Spec required** No |
| **Plan refs** | `docs/UI-UX/05-DESIGN-SYSTEM.md` § Governance, `docs/UI-UX/07-COMPONENT-SPECIFICATION.md`, `docs/PLAN/06-FRONTEND-ARCHITECTURE.md` |

**Goal** — A place where components live, are viewed in every state, and are reviewed — so `docs/UI-UX/05`'s governance rule ("any new token or component must be added here before being used in a page spec") is enforceable rather than aspirational.
...
5. Document each component with its variants, states, and the **rule** governing when to use it — `docs/UI-UX/07` gives a rule per component, and the rule is the part that gets forgotten.
6. Wire the workbench into CI so a component with an undocumented state fails review.

**Definition of Done**
- [ ] Every component from `docs/UI-UX/07` has a workbench entry covering all its documented states.
- [ ] Each entry restates the usage rule from `docs/UI-UX/07`, not just the props.
...
| **Plan refs** | `docs/UI-UX/07-COMPONENT-SPECIFICATION.md` § Button, `docs/UI-UX/15-FORM-UX.md`, `docs/UI-UX/13-ACCESSIBILITY.md` |

**Goal** — Three variants mapped one-to-one to action severity, so an admin can gauge risk from color before reading the label (`docs/UI-UX/05`).
...
3. Loading state replaces the label with a spinner **at the same width** — no layout shift (`docs/UI-UX/07`).
4. Never render a silently disabled button: the component requires a reason, surfaced via tooltip or adjacent helper text (`docs/UI-UX/15`).
5. Encode `docs/UI-UX/07`'s subtlety about visual weight: a `destructive` button must not dominate a screen whose common path is non-destructive. "Deactivate user" on a user detail screen is `secondary` with danger-colored text; the full `destructive` weight is reserved for genuinely rare, high-consequence confirmations like Project Grant revocation.
...
7. Meet minimum target size even inside compact table rows (`docs/UI-UX/13`).
...
- [ ] Target size meets `docs/UI-UX/13`'s minimum at compact density.
...
|---|---|
| **Status** | TODO · **Depends on** PF-02 · **Surface** console · **Spec required** No |
| **Plan refs** | `docs/UI-UX/07-COMPONENT-SPECIFICATION.md` § Table, `docs/UI-UX/14-EMPTY-LOADING-ERROR-STATES.md`, `docs/UI-UX/12-RESPONSIVE-BEHAVIOR.md`, `docs/UI-UX/11-MICRO-INTERACTIONS.md` |

**Goal** — The console's primary data-display pattern, built once. `docs/UI-UX/08` is explicit: no screen invents its own table.
...
2. Four states: default; loading via **skeleton rows matching row height**, never a blocking overlay; empty; and error as an inline banner above the table that keeps showing last-known-good data rather than going blank (`docs/UI-UX/07`, `docs/UI-UX/14`).
3. The kebab reveals on row hover and stays hidden otherwise (`docs/UI-UX/11`) — but is always keyboard-reachable and always announced as "Actions for [row name]", never bare "button" (`docs/UI-UX/13`).
4. Support the role-source badge as a column or inline indicator, since `docs/UI-UX/07` makes it a cross-cutting requirement rather than a per-table choice.
5. Cursor pagination against `docs/PLAN/05`'s `next_page_token`, stable while rows are being inserted.
6. Responsive degradation per `docs/UI-UX/12`: hide secondary columns behind a show-more toggle or expandable row **before** ever requiring horizontal scroll for primary ones.
7. Sorting and filtering update instantly with no row animation (`docs/UI-UX/10`).
...
- [ ] A load error preserves last-known data with a retry that re-fetches only the failed request (`docs/UI-UX/09`).
...
|---|---|
| **Status** | TODO · **Depends on** PF-02 · **Surface** console · **Spec required** No |
| **Plan refs** | `docs/UI-UX/07-COMPONENT-SPECIFICATION.md` § Form Field, `docs/UI-UX/15-FORM-UX.md`, `docs/UI-UX/13-ACCESSIBILITY.md`, `docs/UI-UX/11-MICRO-INTERACTIONS.md` |
...
3. **Labels are always visible.** A placeholder is never a label substitute — placeholders carry format examples only, e.g. `https://app.example.com/callback` under a visible "Redirect URI" label (`docs/UI-UX/15`).
4. Programmatically associate label, helper text, and error text with the input, so a screen reader announces the error when it appears rather than only rendering it visually (`docs/UI-UX/13`).
5. Multi-value list removal is lightweight and offers a brief inline undo, because removing an item from a draft form has no effect until save — deliberately distinct from the heavier confirmation pattern for live actions (`docs/UI-UX/15`).
6. Toggle switches apply immediately on change with brief inline confirmation, and carry no Save button (`docs/UI-UX/11`).
7. Async field validation shows a small inline indicator beside the field, never a full-form block (`docs/UI-UX/15`).
8. Target sizes meet `docs/UI-UX/13`'s minimum, including checkboxes in dense role-selection lists.
...
| **Plan refs** | `docs/UI-UX/07-COMPONENT-SPECIFICATION.md` § Modal / Side Panel, `docs/UI-UX/09-INTERACTION-DESIGN.md` § Keyboard & Focus, `docs/UI-UX/10-MOTION-DESIGN.md`, `docs/UI-UX/12-RESPONSIVE-BEHAVIOR.md` |
...
2. **Side panel**: slides from the right, for multi-step or detail-heavy flows ("Invite user", which `docs/UI-UX/04` Flow 1 makes a two-step progression). Motion: slide.
3. Enforce `docs/UI-UX/07`'s rule in the API: **destructive confirmations always use the modal variant**, never the side panel, so they interrupt rather than blend into a flow.
4. Focus management per `docs/UI-UX/09`: opening moves focus inside immediately; closing returns focus to the triggering element; Escape closes; the focus trap always has a keyboard-accessible exit.
5. At tablet width, a side panel becomes a full-screen takeover — a narrow panel is too cramped for a two-step flow (`docs/UI-UX/12`).
...
|---|---|
| **Status** | TODO · **Depends on** PF-02 · **Surface** console · **Spec required** No |
| **Plan refs** | `docs/UI-UX/07-COMPONENT-SPECIFICATION.md` § Badge / Tag, `docs/UI-UX/06-VISUAL-LANGUAGE.md`, `docs/UI-UX/13-ACCESSIBILITY.md` |

**Goal** — Status and role-source communicated reliably, never by color alone.

**Steps**
1. **Status badge**: color + icon + text, per `docs/UI-UX/06`'s status table — active, invited, deactivated, locked, revoked, draft.
...
3. Enforce the rule in the component's API: text is required, so a badge cannot be constructed as icon-and-color only (`docs/UI-UX/13`).
...
**Definition of Done**
- [ ] Every status in `docs/UI-UX/06`'s table has a badge with color, icon, and text.
...
| **Plan refs** | `docs/UI-UX/07-COMPONENT-SPECIFICATION.md` § Confirmation Dialog, `docs/UI-UX/09-INTERACTION-DESIGN.md` § Consequence Before Confirmation, `docs/UI-UX/04-USER-FLOWS.md` |
...
**Steps**
1. Anatomy per `docs/UI-UX/07`: a title stating the action plainly ("Revoke Project Grant?"), a **consequence summary in plain language** — never a generic "are you sure?" — a `destructive`-styled confirm button labelled with the real verb ("Revoke", not "OK"), and a `secondary` cancel.
...
5. No entrance animation (`docs/UI-UX/10`).
6. Provide the explicit success state that `docs/UI-UX/09`'s feedback table requires for high-risk actions — a silently updated list is insufficient.
...
|---|---|
| **Status** | TODO · **Depends on** PF-02 · **Surface** console · **Spec required** No |
| **Plan refs** | `docs/UI-UX/07-COMPONENT-SPECIFICATION.md` § Breadcrumb, `docs/UI-UX/03-INFORMATION-ARCHITECTURE.md` § Breadcrumb & Wayfinding |

**Goal** — Wayfinding that reflects the real data hierarchy, which `docs/UI-UX/03` says matters most for admins moving between "Projects" and "Granted Projects" — where knowing *whose* project you are looking at is a correctness question, not a convenience.

**Steps**
1. Render the actual data hierarchy: `Acme Org / POS Project / Roles`.
2. Every segment is clickable and jumps directly there.
3. Disambiguate owned versus delegated projects visibly in the trail — this is the specific confusion `docs/UI-UX/03` names.
...
|---|---|
| **Status** | TODO · **Depends on** PF-02 · **Surface** console · **Spec required** No |
| **Plan refs** | `docs/UI-UX/07-COMPONENT-SPECIFICATION.md` § Search Input, `docs/UI-UX/11-MICRO-INTERACTIONS.md` § Search-as-You-Type, `docs/UI-UX/03-INFORMATION-ARCHITECTURE.md` § Search |
...
3. Show a loading indicator **only** if results take longer than a brief instant — a spinner flickering on every keystroke is worse than none (`docs/UI-UX/11`).
...
|---|---|
| **Status** | TODO · **Depends on** PF-02 · **Surface** console · **Spec required** No |
| **Plan refs** | `docs/UI-UX/07`, `docs/UI-UX/11-MICRO-INTERACTIONS.md`, `docs/UI-UX/15-FORM-UX.md` § Multi-Step Forms, `docs/UI-UX/18-DETAILED-PAGE-SPECIFICATIONS.md` |

**Goal** — The components `docs/UI-UX/` requires by implication but does not give their own section: without these, each page invents its own.
...
2. **Step indicator** — "Step 1 of 2", visible at all times in multi-step flows, with data retained on back-navigation and a full summary before the final confirm (`docs/UI-UX/15`).
3. **Toast / inline success** — fade plus slight slide, auto-dismiss with generous read time, announced via a live region so screen-reader users know an action succeeded (`docs/UI-UX/10`, `docs/UI-UX/13`).
4. **Copy-to-clipboard** — prominent, with clear success feedback ("Copy" → "Copied"). `docs/UI-UX/11` singles this out because a silently failed copy of a one-time client secret means regenerating it and updating every consumer app.
5. **KPI card** — for the Organization Overview. Critically: **not clickable-looking unless it has a real drill-down destination** (`docs/UI-UX/09`).
6. **Pagination footer** — shared by every table, cursor-based.
7. **Inline error banner** — the table-load-failure treatment from `docs/UI-UX/14`, with a retry that re-fetches only the failed request and never reloads the page.
...
| **Plan refs** | `docs/UI-UX/08-PAGE-SPECIFICATIONS.md` § Cross-Screen Requirements, `docs/PLAN/08-AUTHORIZATION.md`, `docs/PLAN/06-FRONTEND-ARCHITECTURE.md`, `docs/UI-UX/03-INFORMATION-ARCHITECTURE.md` § Depth Rule |

**Goal** — Routes that a user's claims don't permit are **genuinely unreachable**, not merely hidden by CSS — the rule `docs/UI-UX/08` states as non-negotiable.

**Steps**
1. Build the route tree from `docs/UI-UX/03`'s IA, respecting its **three-level depth rule**: Organization → Projects → [Project] → tab. A fourth level is a signal to reconsider the IA, not to add nesting.
2. Attach a required-permission declaration to each route, expressed in the same manager-role and claim vocabulary the API enforces (`docs/PLAN/08`).
...
| **Plan refs** | `docs/UI-UX/03-INFORMATION-ARCHITECTURE.md`, `docs/UI-UX/12-RESPONSIVE-BEHAVIOR.md`, `docs/PLAN/06-FRONTEND-ARCHITECTURE.md` |

**Steps**
1. Persistent left navigation matching `docs/UI-UX/03`'s structure, with Instance separated at the top rather than nested inside an organization — nesting it would imply it is "one organization among others," which is wrong.
2. Collapse to an icon-only rail at tablet width, expandable on demand (`docs/UI-UX/12`).
...
4. Show "Granted Projects" as its own top-level item, never a tab under Projects — for a receiving organization these are conceptually someone else's projects (`docs/UI-UX/03`).
...
**Definition of Done**
- [ ] Navigation matches `docs/UI-UX/03`'s structure exactly.
...
| **Plan refs** | `docs/UI-UX/03-INFORMATION-ARCHITECTURE.md` § Search, `docs/UI-UX/00-DESIGN-DIRECTION.md` |
...
4. **Scope results server-side by the caller's permissions.** Search must never confirm the existence of a resource the user cannot access; that is an enumeration primitive (`docs/SECURITY/02` §12).
...
| **Plan refs** | `docs/UI-UX/09-INTERACTION-DESIGN.md` § Progressive Disclosure, `docs/UI-UX/01-USER-PERSONAS.md` |
...
**Steps**
1. Hide advanced navigation items — Project Grants, ABAC Policies — until the organization has actually created at least one, **or** the admin holds an applicable manager role (`docs/UI-UX/09`).
...
| **Plan refs** | `docs/UI-UX/14-EMPTY-LOADING-ERROR-STATES.md`, `docs/UI-UX/09-INTERACTION-DESIGN.md` |

**Goal** — Loading, empty, error, and permission-denied handled by shared machinery, so `docs/UI-UX/19`'s chain has something concrete to point at on every screen.

**Steps**
1. Implement **state priority** exactly as `docs/UI-UX/14` specifies: loading beats empty; error beats both once a request has definitively failed. A screen must never show "No results" while a request is in flight.
...
3. **Distinguish genuinely empty from filtered-to-empty.** The filtered case offers "clear filters", never a create action — offering to create a user when the real problem is an over-narrow search is actively unhelpful (`docs/UI-UX/14`).
4. Three distinct error treatments per `docs/UI-UX/14`'s table: list-load failure (inline banner, keep stale data, retry), form validation failure (mapped to fields), form server or network failure (top-of-form banner, **input preserved**, retry).
...
7. Retry re-attempts only the failed request and never reloads the page, preserving scroll position (`docs/UI-UX/09`).
...
| **Plan refs** | `docs/UI-UX/15-FORM-UX.md`, `docs/UI-UX/11-MICRO-INTERACTIONS.md`, `docs/PLAN/05-API-CONTRACT.md` § Standard Error Format, `docs/UI-UX/13-ACCESSIBILITY.md` |

**Goal** — Forms behave identically everywhere, and a server-side validation error lands on the field that caused it.

**Steps**
1. **Validation timing**: on blur, not on every keystroke; and on submit re-validate everything regardless of blur history, to catch fields never touched — browser autofill is the common case (`docs/UI-UX/15`).
2. Map `docs/PLAN/05`'s `details[].field` entries back to their fields automatically. A generic top-of-form banner when field-level detail was available is a defect, not a fallback.
3. On submit with errors, move focus to the **first** invalid field and announce a summary to screen readers (`docs/UI-UX/15`, `docs/UI-UX/13`).
...
5. Enforce `docs/UI-UX/15`'s save-pattern rule at the component level: a form is either explicit-Save or apply-on-change, **never a mix**. Mixing leaves the admin unsure whether their last change saved.
...
7. Preserve input across navigation and failure; never silently discard entered data (`docs/UI-UX/09` § Error Recovery).

**Definition of Done**
- [ ] Blur and submit validation behave per `docs/UI-UX/15`.
...
|---|---|
| **Status** | TODO · **Depends on** PF-13 · **Surface** console · **Spec required** No |
| **Plan refs** | `docs/UI-UX/12-RESPONSIVE-BEHAVIOR.md`, `docs/UI-UX/16-MOBILE-UX.md` |
...
2. Require each page task to **state which columns its regions occupy at each breakpoint** — `docs/UI-UX/12` explicitly rejects "it's responsive" as a specification.
...
4. Below roughly 600px, admin screens show an explicit **unsupported-width message suggesting a larger screen** — `docs/UI-UX/12` is clear that this is better than a silently broken layout.
5. Exempt the mobile-in-scope screens from that boundary: Personal account settings and the hosted login/MFA screens (`docs/UI-UX/16`).
6. Exempt the Rego editor from below-tablet support entirely — policy editing is not a small-screen task regardless of optimization (`docs/UI-UX/12`).
7. Test at the four widths `docs/UI-UX/12` names: 1440px+, 1024–1439px, ~900px, and below 600px.

**Definition of Done**
- [ ] The grid matches `docs/UI-UX/12`'s table exactly.
...
|---|---|
| **Status** | TODO · **Depends on** PF-02 · **Surface** console · **Spec required** No |
| **Plan refs** | `docs/UI-UX/13-ACCESSIBILITY.md`, `docs/UI-UX/17-UX-ACCEPTANCE-CRITERIA.md`, `docs/PLAN/02-REQUIREMENTS.md` (WCAG 2.1 AA) |

**Goal** — Make accessibility a property of the foundation rather than a Phase 5 remediation project. `docs/UI-UX/13` is explicit that retrofitting is far more expensive, and `docs/PLAN/18` R-10 lists exactly this as a medium risk.
...
2. Build a contrast-validation utility, and apply it at the point an org admin **sets** a custom accent color — `docs/UI-UX/13` requires validation at that moment, not a silent inaccessible ship.
...
6. Enforce minimum target sizes at the component level, so compact density and accessibility do not end up in conflict (`docs/UI-UX/13`).
...
| **Plan refs** | `docs/PLAN/06-FRONTEND-ARCHITECTURE.md` § Tech Stack, `docs/PLAN/05-API-CONTRACT.md`, `docs/UI-UX/09-INTERACTION-DESIGN.md` |
...
5. Restrict optimistic updates to the low-risk, reversible actions `docs/UI-UX/09`'s feedback table permits, with rollback on error. Session revocation is the canonical example (`docs/UI-UX/19`'s worked example); access-control mutations are **not** candidates, per the confirm-before-not-undo-after rule.
...
Every task below inherits: the `docs/UI-UX/19` chain table, the twelve console-wide rules, and the column-occupancy documentation required by `PF-18`.
...
| **Plan refs** | `docs/UI-UX/18-DETAILED-PAGE-SPECIFICATIONS.md` § Organization Overview, `docs/UI-UX/08`, `docs/UI-UX/02-USER-JOURNEYS.md` |

**Goal** — The workspace landing screen. `docs/UI-UX/18` specifies it in detail — follow that spec rather than re-deriving it.

**Steps**
1. Implement `docs/UI-UX/18`'s above-the-fold priorities and visual hierarchy exactly.
2. KPI cards using `PF-11`'s component — and only clickable-looking where a real drill-down exists (`docs/UI-UX/09`).
...
**Definition of Done**
- [ ] Matches `docs/UI-UX/18`'s detailed spec, including above-the-fold ordering.
...
| **Plan refs** | `docs/UI-UX/08-PAGE-SPECIFICATIONS.md` (Project list) |
...
| **Plan refs** | `docs/UI-UX/08` (Applications tab), `docs/UI-UX/11-MICRO-INTERACTIONS.md` § Copy-to-Clipboard, `docs/UI-UX/15-FORM-UX.md`, `docs/PLAN/09-SECURITY.md` |
...
3. **The one-time secret reveal**: a modal shown once at creation, with a prominent copy button, unmistakable success feedback, and copy stating plainly that the secret cannot be retrieved again (`docs/UI-UX/08`, `docs/UI-UX/11`).
...
| **Plan refs** | `docs/UI-UX/18-DETAILED-PAGE-SPECIFICATIONS.md` § Users List, `docs/UI-UX/04-USER-FLOWS.md` Flow 1, `docs/UI-UX/08` |

**Goal** — Where admins spend most of their time, plus the invite flow — `docs/UI-UX/04` Flow 1, implemented step by step.

**Steps**
1. Follow `docs/UI-UX/18`'s detailed spec, including its row-action pattern conventions, which other screens then reference.
...
4. **Invite flow as a side panel** with two steps (`docs/UI-UX/07`, `docs/UI-UX/04` Flow 1), step indicator visible, data retained on back-navigation, full summary before confirm.
5. Async email-in-use validation with the inline indicator, not a full-form block (`docs/UI-UX/15`).
...
- [ ] Deactivation's visual weight matches `docs/UI-UX/07`'s rule.
...
| **Plan refs** | `docs/UI-UX/08` (User detail — Profile tab), `docs/UI-UX/15-FORM-UX.md` |
...
| **Plan refs** | `docs/UI-UX/08` (Audit Log), `docs/UI-UX/14`, `docs/PLAN/13-OBSERVABILITY.md` |
...
6. Filtering and export polish are deliberately `PF-50`-era work (`P5-10`); this task delivers the readable baseline `docs/PLAN/17` Phase 1 requires.
...
| **Plan refs** | `docs/UI-UX/08` (Organization Settings), `docs/UI-UX/05-DESIGN-SYSTEM.md` § Color, `docs/UI-UX/13-ACCESSIBILITY.md`, `docs/PLAN/08-AUTHORIZATION.md` Part B |

**Goal** — Branding and domain verification, with the branding constraints enforced rather than documented.

**Steps**
1. Logo upload with type, size, and dimension validation. Treat it as untrusted file input (`docs/SECURITY/02` §9) — the backend validates independently.
2. Accent color picker that **validates contrast at the point of selection** and refuses an inaccessible value with an explanation (`docs/UI-UX/13`).
3. Make it structurally impossible to override `color-danger` or `color-warning` — those must stay universally recognizable regardless of tenant branding (`docs/UI-UX/05`).
...
| **Plan refs** | `docs/UI-UX/08` (Organization list, Instance-wide policies, Instance audit log), `docs/UI-UX/03-INFORMATION-ARCHITECTURE.md`, `docs/PLAN/08-AUTHORIZATION.md` Part C |
...
3. **Instance audit log**: the cross-organization view, reusing `PF-26`'s components with an organization column and filter. It reads through the explicit, audited instance-level access path (`docs/PLAN/04`) — make that visible in the UI, since using it is itself an audited event.
...
| **Plan refs** | `docs/UI-UX/08` (Organization switcher), `docs/PLAN/01-PRODUCT-SCOPE.md` § Out of Scope, `docs/PLAN/06-FRONTEND-ARCHITECTURE.md` |

**Goal** — Context switching for an admin who administers several organizations — **not** the multi-organization end-user workspace switching that `docs/PLAN/01` explicitly defers. These are different features and must not be conflated.

**Steps**
1. Show the switcher only when the signed-in admin actually administers more than one organization.
2. Search input plus list, per `docs/UI-UX/08`'s composition.
...
| **Plan refs** | `docs/UI-UX/08` (Roles tab), `docs/UI-UX/15-FORM-UX.md`, `docs/PLAN/08-AUTHORIZATION.md` Part A |
...
| **Plan refs** | `docs/UI-UX/08` (Authorizations tab, User detail — Grants tab), `docs/UI-UX/06-VISUAL-LANGUAGE.md`, `docs/PLAN/08-AUTHORIZATION.md` |

**Goal** — Assign and revoke project roles, with the role-source badge in place from day one — `docs/UI-UX/08` makes it mandatory on every screen showing roles.
...
| **Plan refs** | `docs/UI-UX/08` (Policies — Access tab), `docs/UI-UX/11-MICRO-INTERACTIONS.md` § Toggle Switches, `docs/PLAN/08-AUTHORIZATION.md` Part B |
...
2. Toggles apply immediately with inline confirmation (`docs/UI-UX/11`); multi-field groups use explicit Save. **Never mix the two within one form** (`docs/UI-UX/15`) — group them into visually separate sections if both are present.
...
**Definition of Done**
- [ ] Fields map exactly to `docs/PLAN/08` Part B's documented JSON shape.
...
| **Plan refs** | `docs/UI-UX/19-FRONTEND-IMPLEMENTATION-CHAIN.md` § Worked Example, `docs/UI-UX/04-USER-FLOWS.md` Flow 4, `docs/UI-UX/11-MICRO-INTERACTIONS.md`, `docs/UI-UX/16-MOBILE-UX.md` |

**Goal** — `docs/UI-UX/19` specifies this screen's revoke button down to its screen-reader label as its worked example. Follow it row by row rather than re-deriving it.

**Steps**
1. Revoke button: `secondary` variant per row, **single click with no modal** — low-risk per `docs/UI-UX/09`'s feedback-timing table — with optimistic removal and rollback on error, a spinner at constant width, and an inline error beside the row on failure.
2. Screen-reader label is **"Revoke session on [device/browser]"**, not bare "Revoke" — `docs/UI-UX/19` calls this out specifically because multiple sessions are listed.
3. Table columns: device, coarse location, created, last active. **"Last active" is relative** ("2 minutes ago"), not absolute (`docs/UI-UX/11`).
4. **Mark the current session distinctly** ("This device") so a user reviewing sessions doesn't revoke the one they are using — `docs/UI-UX/11` names this explicitly.
...
**Definition of Done**
- [ ] The implementation matches `docs/UI-UX/19`'s worked example on all twelve chain rows.
...
| **Plan refs** | `docs/UI-UX/08` (MFA tab), `docs/UI-UX/16-MOBILE-UX.md`, `docs/UI-UX/13-ACCESSIBILITY.md` |

**Goal** — Admins see status read-only; users manage their own factors fully — the split `docs/UI-UX/08` specifies.
...
3. TOTP enrollment shows a QR code **and a copyable manual entry code**. `docs/UI-UX/16` gives the reason: on mobile, the user may be trying to scan a code displayed on the very device that must scan it, which is impossible.
4. QR code needs a text alternative to satisfy `docs/UI-UX/13`; the manual code doubles as it.
...
7. Distinct messages per error: wrong code, expired challenge, unsupported browser (`docs/UI-UX/14`).
...
| **Plan refs** | `docs/UI-UX/16-MOBILE-UX.md`, `docs/UI-UX/08` § Responsive Scope, `docs/UI-UX/12-RESPONSIVE-BEHAVIOR.md` |

**Goal** — The self-service surface, and the **only** console screen group requiring full mobile optimization — because end users, unlike admins, will reach it from a phone, often immediately after realizing they lost a device (`docs/UI-UX/16`, `docs/UI-UX/02` Journey 4).
...
3. **Single-column, full-width layout** at mobile; no side-by-side panels (`docs/UI-UX/16`).
4. **Bottom-anchored primary actions** for thumb reachability, rather than top-of-screen buttons requiring an awkward reach.
5. Session revocation stays single-tap and immediately effective — `docs/UI-UX/16` notes the low-friction requirement matters *more* on mobile, since this is where a security-anxious user reaches first.
...
8. Touch targets meet or exceed `docs/UI-UX/13`'s minimums.

**Definition of Done**
- [ ] Every action works on a real mobile viewport, tested on at least one iOS and one Android device or accurate emulator (`docs/UI-UX/16` § Testing).
...
| **Plan refs** | `docs/UI-UX/18-DETAILED-PAGE-SPECIFICATIONS.md` § Project Grants Tab, `docs/UI-UX/04-USER-FLOWS.md` Flow 2, `docs/UI-UX/11-MICRO-INTERACTIONS.md`, `docs/UI-UX/05-DESIGN-SYSTEM.md` |

**Goal** — The granting side of delegation, implementing Flow 2 exactly. `docs/UI-UX/18` specifies this page in detail.
...
3. Implement `docs/UI-UX/11`'s role-checkbox micro-interaction: checking or unchecking a role **updates the confirmation preview text in real time**, so the plain-language consequence builds up as selections are made rather than appearing only at the end.
4. Use `color-warning` for the state `docs/UI-UX/05` names by example — "this Project Grant has no roles selected yet." Warning, not danger: it is a caution, not a destructive action.
5. Revocation uses the **typed-confirmation** variant (`docs/UI-UX/08` specifies this exact variant), stating that every user holding roles through the grant loses access.
...
| **Plan refs** | `docs/UI-UX/08` (Granted Projects list), `docs/UI-UX/04-USER-FLOWS.md` Flow 3, `docs/UI-UX/11-MICRO-INTERACTIONS.md`, `docs/UI-UX/01-USER-PERSONAS.md` |

**Goal** — The receiving side, implementing Flow 3 — with the hard constraint from `docs/UI-UX/08` that this screen **never shows non-granted roles**.

**Steps**
1. List projects delegated *to* this organization, naming the granting organization and the available roles.
2. **A role outside `granted_role_keys` is simply absent** — never a disabled checkbox. `docs/UI-UX/11` is explicit: a disabled checkbox implies "you could have this, but not right now," which is the wrong signal entirely.
...
| **Plan refs** | `docs/UI-UX/08` (Policies — ABAC tab), `docs/UI-UX/04-USER-FLOWS.md` Flow 5, `docs/UI-UX/12-RESPONSIVE-BEHAVIOR.md`, `docs/UI-UX/13-ACCESSIBILITY.md` |
...
5. Version history with single-action rollback per version (`docs/PLAN/17` Phase 4b requires reverting to be one action).
6. `color-danger` only for genuinely destructive actions — an activation that would deny, and deletion.
7. **Choose the editor component for accessibility up front.** It must be keyboard-navigable and screen-reader usable, or it fails `docs/UI-UX/13`'s AA requirement — discovering this during the Phase 5 audit means replacing the editor late.
8. Below tablet width, this screen may require a larger screen — `docs/UI-UX/12` explicitly exempts it, since policy editing is not a small-screen task.
...
These are served by the auth service itself, not the console SPA (`P1-12`) — they must work with JavaScript disabled and must not depend on the console's deploy cycle. They are designed under the same mobile principles as the console's in-scope screens, since end users log in from phones constantly (`docs/UI-UX/16`).
...
| **Plan refs** | `docs/UI-UX/15-FORM-UX.md`, `docs/UI-UX/13-ACCESSIBILITY.md`, `docs/UI-UX/16-MOBILE-UX.md`, `docs/SECURITY/02` §5, §6, §12 |

**Steps**
1. Server-rendered, functioning without client-side JavaScript.
2. Field anatomy and error presentation per `docs/UI-UX/15`, with an error summary linked to fields.
3. **Uniform error messaging**: a wrong password and a nonexistent account are indistinguishable in text, status, and timing (`docs/SECURITY/02` §12).
...
7. Never reflect the `error` or `state` parameter into the page unescaped (`docs/SECURITY/02` §6).
...
| **Plan refs** | `docs/UI-UX/16-MOBILE-UX.md`, `docs/UI-UX/15-FORM-UX.md`, `docs/UI-UX/13-ACCESSIBILITY.md` |
...
| **Plan refs** | `docs/UI-UX/15-FORM-UX.md`, `docs/PLAN/09-SECURITY.md`, `docs/SECURITY/02` §12 |

**Steps**
1. Reset request form whose response is **identical whether or not the email exists** (`docs/SECURITY/02` §12).
...
| **Plan refs** | `docs/PLAN/05-API-CONTRACT.md` § Session & logout, `docs/UI-UX/14-EMPTY-LOADING-ERROR-STATES.md`, `docs/SECURITY/02` §5 |
...
| **Plan refs** | `docs/UI-UX/20-PUBLIC-SITE-SPECIFICATIONS.md` § Design Direction Differences, `docs/UI-UX/06-VISUAL-LANGUAGE.md`, `docs/PLAN/20-PUBLIC-SITE-ARCHITECTURE.md` |

**Goal** — A site that shares the brand language with the console and **shares no code with it** — the separation `docs/PLAN/20` insists on.

**Steps**
1. Implement the design direction differences `docs/UI-UX/20` specifies. This is a marketing surface, not an admin tool: the console's density-over-whitespace principle does not transfer.
2. Duplicate the brand-level tokens from `docs/UI-UX/06` deliberately. Do **not** import the console's component library — sharing it would force compromises in both directions, which is exactly why the two are separate projects.
3. Header, footer, and navigation for an anonymous visitor, whose needs have almost nothing in common with an authenticated admin's (`docs/UI-UX/03` scope note).
4. Meet the cross-page requirements in `docs/UI-UX/20`, including performance and SEO baselines.
...
- [ ] Layout matches `docs/UI-UX/20`'s design direction.
- [ ] Lighthouse performance and SEO meet `docs/UI-UX/20`'s bar.
...
| **Plan refs** | `docs/UI-UX/20-PUBLIC-SITE-SPECIFICATIONS.md` § Detailed Spec: Landing Page, `docs/UI-UX/21-CONTENT-AND-COPY-STRATEGY.md`, `CLAUDE.md` |

**Steps**
1. Build to `docs/UI-UX/20`'s detailed landing page spec, using `docs/UI-UX/21`'s copy blueprints — which contain real example copy, not placeholders.
...
4. Optimize the first paint; this is an SEO and bounce-rate surface (`docs/PLAN/20`).
5. No console code, no console component imports.

**Definition of Done**
- [ ] Matches `docs/UI-UX/20`'s spec and `docs/UI-UX/21`'s copy blueprints.
...
|---|---|
| **Status** | TODO · **Gate** P0-18 · **Size** M · **Surface** public-site |
| **Plan refs** | `docs/UI-UX/20-PUBLIC-SITE-SPECIFICATIONS.md` § Page Inventory, `docs/UI-UX/21-CONTENT-AND-COPY-STRATEGY.md`, `docs/PLAN/20-PUBLIC-SITE-ARCHITECTURE.md` |

**Steps**
1. About page per `docs/UI-UX/21`'s blueprint.
2. Contact page carrying the **responsible-disclosure channel** — `docs/PLAN/20` requires external researchers to have a documented path, and it must exist from the first publish (`P0-19`).
...
5. Any contact form is spam-resistant without a tracking-heavy third-party widget, consistent with the privacy-respecting analytics choice in `docs/PLAN/20`.

**Definition of Done**
- [ ] All three pages match their `docs/UI-UX/21` blueprints.
...
| **Plan refs** | `docs/UI-UX/20-PUBLIC-SITE-SPECIFICATIONS.md` § Docs Home, § API Reference, `docs/PLAN/20-PUBLIC-SITE-ARCHITECTURE.md` § API Reference Generation, `CLAUDE.md` |

**Goal** — A docs experience good enough that fast search is not a differentiator but a baseline — `docs/PLAN/20` calls it make-or-break for adoption.

**Steps**
1. Docs shell per `docs/UI-UX/20`'s Docs Home spec: sidebar generated from folder structure, versioning, search.
...
4. Docs versioning working with the current API version as default, and older versions reachable through their deprecation window (`docs/PLAN/20`).
...
7. Meet `docs/UI-UX/20`'s API Reference spec for how endpoints, parameters, and examples are presented.
...
| **Plan refs** | `docs/PLAN/20-PUBLIC-SITE-ARCHITECTURE.md` § What Never Gets Published, `docs/UI-UX/20`, `docs/UI-UX/21` |
...
2. Honor `docs/PLAN/20`'s prohibitions absolutely. **Never publish**: attack scenarios or mitigation mechanics from `docs/SECURITY/02`; infrastructure topology from `docs/PLAN/14`; anything from `docs/PLAN/18-RISK-REGISTER.md`.
...
**Definition of Done**
- [ ] Published and matching `docs/UI-UX/20`'s spec.
- [ ] A review confirms no `docs/SECURITY/02`, `docs/PLAN/14` topology, or `docs/PLAN/18` content appears.
...
| **Plan refs** | `docs/PLAN/11-TESTING.md`, `docs/PLAN/06-FRONTEND-ARCHITECTURE.md` § Testing |
...
2. Prioritize the logic `docs/PLAN/11` names for the frontend: form validation, redirect URI format, role key format.
...
5. Contract-test against the generated client, so a backend change not reflected in the spec breaks CI before it breaks the console at runtime (`docs/PLAN/06`).
...
| **Plan refs** | `docs/PLAN/11-TESTING.md` § E2E, `docs/PLAN/06-FRONTEND-ARCHITECTURE.md` § Testing, `docs/UI-UX/04-USER-FLOWS.md` |

**Steps**
1. Cover the flows `docs/PLAN/06` names as highest-risk: user invite plus first role assignment, Project Grant creation and revocation, session revocation.
2. Cover all five `docs/UI-UX/04` flows end to end as their gates open.
...
**Definition of Done**
- [ ] All five `docs/UI-UX/04` flows are covered as their gates open.
- [ ] The three highest-risk flows from `docs/PLAN/06` pass in CI.
...
| **Plan refs** | `docs/UI-UX/13-ACCESSIBILITY.md` § Testing & Sign-off, `docs/UI-UX/17-UX-ACCEPTANCE-CRITERIA.md`, `docs/PLAN/17-ACCEPTANCE-CRITERIA.md` § Phase 5 |

**Goal** — Pass the WCAG 2.1 AA audit that `docs/PLAN/17` makes a Phase 5 gate. This task is `P5-11`'s implementation.

**Steps**
1. Automated axe-core checks across every screen in CI, from `PF-19`'s infrastructure.
2. Audit every screen against `docs/UI-UX/17`'s detailed checklist.
...
**Definition of Done**
- [ ] The console passes a WCAG 2.1 AA audit — `docs/PLAN/17` Phase 5 criterion.
- [ ] Every screen meets `docs/UI-UX/17`'s checklist.
...
| **Plan refs** | `docs/UI-UX/05-DESIGN-SYSTEM.md` § Governance, `docs/UI-UX/12-RESPONSIVE-BEHAVIOR.md` |

**Steps**
1. Snapshot every workbench story at the four `docs/UI-UX/12` test widths.
...
| **Plan refs** | `docs/PLAN/16-IMPLEMENTATION-ROADMAP.md` § Phase 5, `docs/UI-UX/12-RESPONSIVE-BEHAVIOR.md`, `docs/UI-UX/14`, `docs/PLAN/20-PUBLIC-SITE-ARCHITECTURE.md` |
...
4. Eliminate layout shift during loading; skeletons preserve final geometry (`docs/UI-UX/09`, `docs/UI-UX/14`).
5. Optimize the initial load path including the OIDC redirect round-trip — the first thing every user experiences.
6. Hold the public site to a stricter budget than the console: it is an SEO and bounce-rate surface, while the console is an application users are already committed to (`docs/PLAN/20`).
...
| **Plan refs** | `docs/UI-UX/17-UX-ACCEPTANCE-CRITERIA.md`, `docs/PLAN/17-ACCEPTANCE-CRITERIA.md`, `docs/UI-UX/19-FRONTEND-IMPLEMENTATION-CHAIN.md` |

**Steps**
1. Verify every criterion in `docs/UI-UX/17-UX-ACCEPTANCE-CRITERIA.md` with recorded evidence.
...
**Definition of Done**
- [ ] Every `docs/UI-UX/17` criterion is verified with evidence.
...
- [ ] Every component in `docs/UI-UX/07-COMPONENT-SPECIFICATION.md` exists in the library with all documented states and its usage rule.
- [ ] All 21 console screens in `docs/UI-UX/08-PAGE-SPECIFICATIONS.md` are implemented, each gated correctly on its backend task.
...
- [ ] Every screen has a committed `docs/UI-UX/19` implementation-chain table.
- [ ] The console passes WCAG 2.1 AA (`docs/PLAN/17` Phase 5 gate).
