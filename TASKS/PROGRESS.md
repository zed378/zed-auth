...
**Current phase**: Phase 4 — Enterprise Interop (6 / 16). The [Phase 4 threat review](../MEMORY/records/2026-09-15-P3-15-phase-4-threat-review.md)'s T4-1 is decided (ADR-025: the stricter of both organizations' policies applies); T4-2, subset validation at the readers, is built (`P4-04`). Phase 3 complete (15 / 15), tagged `v0.3.0-phase3`, **accepted on staging** — [summary](../MEMORY/records/2026-09-15-P3-phase-3-summary.md). Phase 2 complete (17 / 17), tagged `v0.2.0-phase2`. Phase 0 is 21 / 22; `P0-20` stays WIP pending the pull-based deployment re-read
**Overall**: 88 / 179 tasks done
...
| [Phase 4 — Enterprise Interop](./PHASE-4-ENTERPRISE-INTEROP.md) | 16 | 6 | **ACTIVE** — Phase 3 exit met; [threat review](../MEMORY/records/2026-09-15-P3-15-phase-4-threat-review.md) done; T4-1 decided (ADR-025) | Phase 3 exit + threat model review |
...
| P3-02 | TOTP enrollment | M | **DONE** — the algorithm is in the repository rather than in a dependency (thirty lines, frozen since 2011, and a supply-chain compromise of a library that computes second factors compromises every second factor at once) and is therefore tested against **RFC 6238's own published vectors** rather than against itself. Replay is bounded by recording the COUNTER, never the code — the code is a live credential until its step ends. Corrected a silent deviation: `P3-01` created `user_factors.secret`, `docs/PLAN/04` specifies `user_mfa_factors.secret_encrypted`. `users.mfa_enabled` is now maintained by a trigger, because a denormalized flag that can disagree with its source is worse than no flag. Steps 4 and 8 carried to `P3-12`, which has the endpoint | P3-01 |
| P3-03 | TOTP verification at login | M | **DONE** — the step that makes every enrolled factor mean something, with `docs/PLAN/11`'s literal case (correct password, wrong code, rejected) asserted against the real chain and a positive control beside it. Two findings worth the trouble: `Challenge.PendingID` existed to stop a challenge completing a different authorization request and **nothing compared it** — caught by the first end-to-end run, not by any unit test — and step 4's uniformity requirement **cannot be met by any sequential flow**, since reaching the code page is itself the disclosure that the password was right (`PG-38`). The per-user bound is the control the rest leans on: five per challenge bounds nothing when an attacker holding the password can restart for free. 17 mutations, all red. Step 6 carried to `P3-11` | P3-02 |
| P3-04 | Recovery codes and lost-device process | M | **DONE** — the path back for somebody whose phone is gone, and the first task where the weakest link is made of people rather than code. Two findings: a seam declared `(userID, orgID)` and implemented `(orgID, userID)` **compiled fine** — both are strings — and would have set the tenant to a user id, matched nothing under RLS, and left recovery codes silently never offered; and a comment claiming the alphabet had no confusable characters was simply false, which a test asserting it found. `docs/PLAN/04` asks for Argon2 and gets SHA-256, recorded as `PG-39`: a slow KDF defends low-entropy inputs, and ten memory-hard computations per attempt would be a denial of service on a path an attacker holding the password can drive. A recovery login claims `mfa` and **not** `otp`, so a consumer demanding a device correctly refuses it. The administrator path destroys credentials and mints none; everything else about it is procedural and the runbook says so in those words. 17 mutations, all red. Steps 1 and 3's self-service half carried to `P3-12` | P3-02 |
| P3-05 | WebAuthn / passkey support | L | **DONE except DoD item 1** — a second factor that cannot be phished, and the first task to hit a conflict the plan never acknowledged: `P1-12` made "no JavaScript" load-bearing and WebAuthn is a browser API with no form-only equivalent (`PG-40`). Resolved by containment — `/login` and a TOTP-only challenge are unchanged, and only a page rendering a passkey form carries `script-src`, pinned by hash. A bug only a browser would otherwise have shown: the policy came from the embedded `Page` rather than the `ChallengePage`, so the script would have rendered and then been forbidden. Every assertion in the tests is a genuine ES256 signature from a software authenticator, because a fake verifier cannot prove anything about phishing — the fake would be the thing deciding. 15 mutations, all red; two were redundant guards, and the second exposed a real gap (nothing tested that a pending credential cannot authenticate). **Cross-browser verification is NOT claimed**: it needs hardware this environment does not have | P3-01 |
| P3-06 | Refresh token rotation with reuse detection | M | **DONE** — and the task that found two bugs already shipping, both invisible because both produced working refreshes: every refresh started a NEW family, so `replaced_by` had never been written and reuse detection was impossible rather than absent; and `family_expires_at` was recomputed on each issuance, so continuous refreshing extended a session forever — this card's own step-6 abuse case, live since Phase 1. The hard half is not detection but telling reuse apart from a client retrying after a lost response: get it wrong strictly and real users are logged out until somebody disables the protection, at which point the control is gone AND everybody believes it is there. Two conditions, and it is the untouched successor rather than the clock that distinguishes them. The mutation run then found two of my own tests proving less than their names — a one-second tolerance against a ninety-day lifetime, and a comment overstating a guard. 12 mutations, all red | P1-07 |
| P3-07 | Organization-mandated MFA enforcement | M | **DONE** — closes `docs/PLAN/17`'s Phase 2 criterion that `P2-10` left open because MFA did not exist yet. Neither deny (a support queue, then the setting gets turned off) nor warn (the setting does nothing): route into enrolment at login, with no session until it finishes. The grace is fourteen days from a service-set activation stamp that a caller cannot backdate. **A requirement every test said was met was not**: the enable/disable audit events were defined and the transition logic tested, and nothing emitted either — found by checking callers before writing the record. The mutation run then found two tests proving less than their names, and one fix that silently failed to apply. 15 mutations, all red. A rollback silently disables the mandate, and the record says so | P3-03, P2-10 |
| P3-08 | Login anomaly detection | L | **DONE except DoD item 4** — new device, new location and impossible travel, recorded always and notified only when a deployment turns it on: ADR-024 decides notify over step-up, because the signal is a VPN as often as a thief. The false-positive measurement needs staging, which is unreachable, so notifications ship **off**. **Building the report flow found `/password/set` consuming a token of any purpose** — harmless until this task added a link that must never set a password; fixed before one was ever issued. "Forces a credential change" turned out to mean clearing the password, since ending sessions leaves the stranger a working one. `PG-41`: no plan names a geolocation source; none ships. `P3-06`'s reuse alert had never been written, and now is. 19 mutations; one test passed for the wrong reason and was rebuilt | P1-11, P1-14 |
| P3-09 | Session management API | M | **DONE** — five routes: a person's own sessions under `/v1/me/sessions` (list, end one, end all others) and a member's for `ORG_ADMIN`. The user comes from the token or the scoped path, and a session is always resolved together with its owner. No IP and no raw user agent in any response — browser family, OS and city, from `P3-08`'s parser. Refresh tokens end with their session. **Three bugs found beside it**: every revocation would have tripped the audit guard (fixed by auditing through `management.Audit`, in the transaction); correct no-ops across the API were already tripping it (`management.Unchanged`); and the session cache cached a stale `last_seen_at`, defeating its own write throttle for a minute at a time. 23 mutations, all red | P1-11 |
| P3-10 | Console — MFA tab | M | **DONE** — the tab, and the factor API no card owned (`PG-42`): self-service enrol/confirm/remove/regenerate behind a ten-minute recent sign-in (`REAUTHENTICATION_REQUIRED` sends the user through `prompt=login`), a read-only administrator view, and a hosted passkey page because a passkey cannot be registered from the console's origin (`PG-43`). **Its E2E test found passkeys were never challenged in the running service** — the framework asked a registry the ceremony is not in, so a passkey-only user signed in with a password alone. Also fixed: a forced-enrolment lockout on an abandoned pending factor, and an E2E fixture broken since P3-06's rotation. Both factor types enrolled and used to sign in, in Chromium; 26 E2E; 23 mutations | P3-02, P3-05 |
...
| P3-15 | Phase 3 acceptance validation | M | **DONE** — deployed Phases 2 and 3 to staging (backup, migrate, roll out) and ran all three `docs/PLAN/17` criteria there: 19 checks, 0 failures. **A load-test A/B with the MFA key switched off found every sign-in answering 500** — a nil `*mfa.Framework` inside the login handler's interface, since `P3-03`; fixed at wiring and method, verified on staging with MFA off. Refresh p50 ~60ms vs 50ms target, attributed to rotation (not the mandate check) and accepted for Phase 5. Phase 4 threat review: 14 findings | P3-14 |
...
| P4-07 | SAML 2.0 IdP — core | L | **DONE** — the assertion machinery and the layer that refuses a malicious one: XML bounded before it is parsed (size, depth, DTDs, entities) and round-trip-validated, and `Verify` validates the element about to be CONSUMED rather than the document, which is what makes signature wrapping fail rather than pass. Audience, window and replay are enforced by the package that builds them, and the replay defence IS a primary key. Its own key set by `purpose`, carrying a certificate an OIDC key has no use for. `goxmldsig` upgraded to v1.6.0 for GO-2026-4753 — and the ADR corrected, because an advisory check against a dependency nothing calls proves nothing. Migrations 040 and 041, two fuzz targets, 19 mutations | P1-11 |
| P4-08 | SAML SP- and IdP-initiated flows | L | **DONE** — both flows end to end. SP-initiated on both bindings, verified by verifying the assertion; IdP-initiated opt-in per registration and off by default, carrying no `InResponseTo` because it answers no request. The login page is not edited at all: the request id carries a `saml:` namespace and a dispatcher routes it, so the most security-critical path in the service gains no branch. Three switches were stored and enforced by nothing and now are not — signed `AuthnRequest`s (refused on HTTP-Redirect rather than accepted unverified), the IdP-initiated opt-in, and Single Logout's refusal, which reached only one of its two bindings. Migrations 042–044, 38 mutations, all caught | P4-07 |
| P4-09 | SAML application type and metadata | M | **DONE** — a `saml` application is registered from its partner's metadata or by hand, through the API and the console, in one transaction with its registration. The metadata is untrusted XML and goes through P4-07's gate on the server. `/saml/metadata` now publishes the key that will sign as well as the one that does, so a rotation no longer breaks every service provider at once. Five defects found while building it, all of which compiled — among them a `Published` that silently dropped every `next` key, and a constraint translation that could never execute under pgx. 38 mutations | P4-07, P1-18 |
